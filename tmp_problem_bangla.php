...
$sql = "SELECT v.value_text FROM asset_values v JOIN asset_fields f ON f.id = v.field_id WHERE f.field_key = 'remarks_additional_information' AND v.value_text LIKE '%31 ?????%' LIMIT 1";
$text = (string) db()->query($sql)->fetchColumn();
file_put_contents(__DIR__ . '/storage/runtime/problem_bangla.txt', $text);
echo $text, PHP_EOL;
...
foreach ($fonts as $font) {
    ...
    export_pdf($html, $font . '.pdf', 'portrait', __DIR__ . '/storage/runtime/' . $font . '_problem_remarks_test.pdf');
}
