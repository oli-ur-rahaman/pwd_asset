...
function ensure_asset_schema(): void
{
    ...
    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_common_profiles (
            ...
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT NULL,
            UNIQUE KEY uniq_asset_common_profiles_category (category_id),
...
function asset_upsert_common_profile_binding(int $childFieldId, array $payload): array
{
    ...
    $profile = get_asset_common_profile_by_category($profileCategoryId, null, true);
    if (!$profile) {
        $stmt = db()->prepare('
        ...
        ');
        $stmt->execute([
            asset_normalize_segment_id((int)($payload['segment_id'] ?? 0)),
            $profileCategoryId,
            $definitionMode,
            $rowPolicy,
            $parentSegmentId,
...
{
    $hasScopedRows = false;
    if (($officeType ?? 0) > 0 && ($officeId ?? 0) > 0) {
        $scopeStmt = db()->prepare(
            'SELECT COUNT(*)
             FROM asset_common_admin_rows
             WHERE profile_id = ?
               AND active_status = 1
               AND scope_office_type = ?
               AND scope_office_id = ?'
        );
        $scopeStmt->execute([$profileId, $officeType, $officeId]);
        $hasScopedRows = (int)$scopeStmt->fetchColumn() > 0;
    }
    if ($hasScopedRows) {
        $stmt = db()->prepare(
            'SELECT r.id, r.profile_id, r.stable_row_key, r.scope_office_type, r.scope_office_id, r.sort_order, r.active_status,
             ...
             WHERE r.profile_id = ?
               ...
               AND r.scope_office_type = ?
               AND r.scope_office_id = ?
             ORDER BY r.sort_order ASC, r.id ASC, v.id ASC'
        );
        ...
        $stmt = db()->prepare(
            'SELECT r.id, r.profile_id, r.stable_row_key, r.scope_office_type, r.scope_office_id, r.sort_order, r.active_status,
             ...
             WHERE r.profile_id = ?
               AND r.active_status = 1
               AND r.scope_office_type = 0
               AND r.scope_office_id = 0
             ORDER BY r.sort_order ASC, r.id ASC, v.id ASC'
        );
        $stmt->execute([$profileId]);
    }
...
function asset_common_admin_template_headers(int $profileId): array
{
    $headers = ['Scope', 'Sort Order'];
    foreach (get_asset_common_profile_fields($profileId) as $profileField) {
        $headers[] = (string)($profileField['child_field_label'] ?? 'Field');
    }
    return $headers;
}
...
function build_asset_common_admin_template_workbook(int $profileId): \PhpOffice\PhpSpreadsheet\Spreadsheet
{
    ...
    $scopeOptions = asset_common_scope_options();

    $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
    export_apply_spreadsheet_font($spreadsheet);
    $sheet = $spreadsheet->getActiveSheet();
    $sheet->setTitle('common_rows');
    $dropdownSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'scope_options');
    ...
    $headers = asset_common_admin_template_headers($profileId);
    foreach ($headers as $index => $label) {
        $columnIndex = $index + 1;
        $cell = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($columnIndex) . '1';
        $sheet->setCellValue($cell, $label);
        $sheet->getColumnDimensionByColumn($columnIndex)->setWidth($index === 0 ? 36 : 22);
    }
    $sheet->getStyle('A1:' . \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(count($headers)) . '1')->applyFromArray([
        'font' => ['bold' => true],
        'fill' => [
    ...
    $dropdownSheet->setCellValue('A1', 'Scope');
    $scopeRow = 2;
    foreach ($scopeOptions as $label) {
        $dropdownSheet->setCellValue('A' . $scopeRow, $label);
        $scopeRow++;
    }
    $scopeRange = '$A$2:$A$' . max(2, $scopeRow - 1);
    $spreadsheet->addNamedRange(new \PhpOffice\PhpSpreadsheet\NamedRange('COMMON_SCOPE_OPTIONS', $dropdownSheet, $scopeRange));

    $rowNumber = 2;
    foreach ($existingRows as $row) {
        $sheet->setCellValue('A' . $rowNumber, (string)($row['scope_label'] ?? asset_common_scope_label(0, 0)));
        $sheet->setCellValue('B' . $rowNumber, (int)($row['sort_order'] ?? 0));
        foreach ($profileFields as $offset => $profileField) {
            $childFieldId = (int)($profileField['child_field_id'] ?? 0);
            $column = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($offset + 3);
            $sheet->setCellValueExplicit($column . $rowNumber, (string)($row['values'][$childFieldId] ?? ''), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        }
        $rowNumber++;
    }
    if ($rowNumber === 2) {
        $sheet->setCellValue('A2', 'All Offices (Global)');
        $sheet->setCellValue('B2', 10);
    }

    $validationEnd = max($rowNumber + 25, 100);
    for ($row = 2; $row <= $validationEnd; $row++) {
        $validation = $sheet->getCell('A' . $row)->getDataValidation();
        $validation->setType(\PhpOffice\PhpSpreadsheet\Cell\DataValidation::TYPE_LIST);
        $validation->setFormula1('=COMMON_SCOPE_OPTIONS');
        $validation->setAllowBlank(false);
        $validation->setShowDropDown(true);
        $validation->setShowErrorMessage(true);
        $validation->setErrorTitle('Invalid Scope');
        $validation->setError('Choose a valid scope from dropdown.');
    }
...
function import_asset_common_admin_rows_from_template(int $profileId, array $file): array
{
    ...
    $sheet = $spreadsheet->getSheetByName('common_rows') ?: $spreadsheet->getActiveSheet();
    ...
    foreach ($rows as $index => $row) {
        ...
        $scopeLabel = trim((string)($row[0] ?? ''));
        $sortOrder = (int)($row[1] ?? 0);
        $hasPayload = $scopeLabel !== '' || $sortOrder > 0;
        foreach ($profileFields as $offset => $profileField) {
        ...
        $parsedRows[] = [
            ...
            'sort_order' => max(1, $sortOrder > 0 ? $sortOrder : (($index + 1) * 10)),
            'values' => $values,
        ];
    }
    ...
    db()->beginTransaction();
    try {
        $existingRowIds = db()->prepare('SELECT id FROM asset_common_admin_rows WHERE profile_id = ?');
        $existingRowIds->execute([$profileId]);
        $rowIds = array_map('intval', $existingRowIds->fetchAll(PDO::FETCH_COLUMN) ?: []);
        if ($rowIds) {
            $placeholders = implode(',', array_fill(0, count($rowIds), '?'));
            db()->prepare("DELETE FROM asset_common_admin_row_values WHERE row_id IN ({$placeholders})")->execute($rowIds);
        }
...
function validate_asset_field_definition(array $input, array $fileInput = [], ?int $fieldId = null): array
{
    ...
    if ($commonRowConfigPresent && $isCommonRowField === 1) {
        ...
        $commonProfileCategoryId = (int)($input['common_profile_category_id'] ?? $input['category_id'] ?? 0);
        $commonCategory = $commonProfileCategoryId > 0 ? get_asset_category($commonProfileCategoryId, $segmentId) : null;
        if (!$commonCategory) {
            $errors[] = 'Common row category is required.';
        }

        $existingProfile = $commonCategory ? get_asset_common_profile_by_category((int)$commonCategory['id'], $segmentId, true) : null;
        ...
        if ($commonDefinitionMode === asset_common_definition_mode_user_defined()) {
            ...
            $parentCategory = $commonParentCategoryId > 0 ? get_asset_category($commonParentCategoryId, $commonParentSegmentId) : null;
            $parentField = $commonParentFieldId > 0 ? get_asset_field($commonParentFieldId, $commonParentSegmentId) : null;
            if ($commonParentSegmentId <= 0 || !$parentCategory) {
                $errors[] = 'Parent segment and parent category are required for user-defined common rows.';
            }
            if (!$parentField) {
