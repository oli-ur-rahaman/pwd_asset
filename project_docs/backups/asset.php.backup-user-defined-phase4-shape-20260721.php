...
function ensure_asset_schema(): void
{
    ...
    $initialized = true;
    if (asset_schema_cache_matches() && asset_schema_cache_is_safe_to_trust()) {
        return;
    }
    ...
    db()->exec(
        "CREATE TABLE IF NOT EXISTS segments (
            ...
            new_batch_days INT NOT NULL DEFAULT 0,
    ...
    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_fields (
            ...
            hosted_tutorial_video_path VARCHAR(255) DEFAULT NULL,
            hosted_tutorial_video_original_name VARCHAR(255) DEFAULT NULL,
            hosted_tutorial_video_size BIGINT DEFAULT NULL,
    ...
    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_common_profiles (
            id INT AUTO_INCREMENT PRIMARY KEY,
            segment_id INT NOT NULL,
            category_id INT NOT NULL,
            definition_mode VARCHAR(30) NOT NULL,
            row_policy VARCHAR(20) NOT NULL,
            parent_segment_id INT DEFAULT NULL,
            parent_category_id INT DEFAULT NULL,
            active_status TINYINT NOT NULL DEFAULT 1,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT NULL,
            UNIQUE KEY uniq_asset_common_profiles_segment_category (segment_id, category_id),
            KEY idx_asset_common_profiles_segment (segment_id),
            KEY idx_asset_common_profiles_parent (parent_segment_id, parent_category_id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    );

    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_common_profile_fields (
            id INT AUTO_INCREMENT PRIMARY KEY,
            profile_id INT NOT NULL,
            child_field_id INT NOT NULL,
            parent_field_id INT DEFAULT NULL,
            sort_order INT NOT NULL DEFAULT 0,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT NULL,
            UNIQUE KEY uniq_asset_common_profile_field_child (child_field_id),
            UNIQUE KEY uniq_asset_common_profile_field_profile_child (profile_id, child_field_id),
            KEY idx_asset_common_profile_fields_profile (profile_id),
            KEY idx_asset_common_profile_fields_parent (parent_field_id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    );

    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_common_admin_rows (
            id INT AUTO_INCREMENT PRIMARY KEY,
            profile_id INT NOT NULL,
            stable_row_key VARCHAR(100) NOT NULL,
            scope_office_type INT NOT NULL DEFAULT 0,
            scope_office_id INT NOT NULL DEFAULT 0,
            sort_order INT NOT NULL DEFAULT 0,
            active_status TINYINT NOT NULL DEFAULT 1,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT NULL,
            UNIQUE KEY uniq_asset_common_admin_rows_profile_key (profile_id, stable_row_key),
            KEY idx_asset_common_admin_rows_profile (profile_id),
            KEY idx_asset_common_admin_rows_profile_sort (profile_id, scope_office_type, scope_office_id, sort_order, id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    );

    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_common_admin_row_values (
            id INT AUTO_INCREMENT PRIMARY KEY,
            row_id INT NOT NULL,
            child_field_id INT NOT NULL,
            ...
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT NULL,
            UNIQUE KEY uniq_asset_common_admin_row_value (row_id, child_field_id),
            KEY idx_asset_common_admin_row_values_row (row_id),
            KEY idx_asset_common_admin_row_values_field (child_field_id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    );
    ...
    db()->exec(
        "CREATE TABLE IF NOT EXISTS assets (
            ...
            common_profile_id INT DEFAULT NULL,
            common_row_key VARCHAR(100) DEFAULT NULL,
            common_source_type VARCHAR(30) NOT NULL DEFAULT 'normal',
            common_source_asset_id INT DEFAULT NULL,
            is_user_added_row TINYINT NOT NULL DEFAULT 1,
    ...
    db()->exec(
        "CREATE TABLE IF NOT EXISTS asset_filter_items (
            id INT AUTO_INCREMENT PRIMARY KEY,
            segment_id INT NOT NULL,
            field_id INT NOT NULL,
            field_key VARCHAR(100) NOT NULL,
            ...
            item_sort_value VARCHAR(255) DEFAULT NULL,
            usage_count INT NOT NULL DEFAULT 0,
            active_status TINYINT NOT NULL DEFAULT 1,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT NULL,
            UNIQUE KEY uniq_asset_filter_item (segment_id, field_id, item_kind, item_value),
            KEY idx_asset_filter_items_segment_field (segment_id, field_id, active_status),
            KEY idx_asset_filter_items_segment_key (segment_id, field_key, active_status)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
    );
    ...
    asset_ensure_column('info', 'login_video_link_enabled', 'TINYINT NOT NULL DEFAULT 1');
    ...
    asset_ensure_column('segments', 'new_batch_days', 'INT NOT NULL DEFAULT 0');
    ...
    asset_ensure_column('asset_fields', 'hosted_tutorial_video_path', 'VARCHAR(255) DEFAULT NULL');
    asset_ensure_column('asset_fields', 'hosted_tutorial_video_original_name', 'VARCHAR(255) DEFAULT NULL');
    asset_ensure_column('asset_fields', 'hosted_tutorial_video_size', 'BIGINT DEFAULT NULL');
    ...
    asset_ensure_column('assets', 'common_profile_id', 'INT DEFAULT NULL');
    asset_ensure_column('assets', 'common_row_key', 'VARCHAR(100) DEFAULT NULL');
    asset_ensure_column('assets', 'common_source_type', "VARCHAR(30) NOT NULL DEFAULT 'normal'");
    asset_ensure_column('assets', 'common_source_asset_id', 'INT DEFAULT NULL');
    asset_ensure_column('assets', 'is_user_added_row', 'TINYINT NOT NULL DEFAULT 1');
    asset_ensure_column('asset_common_admin_rows', 'scope_office_type', 'INT NOT NULL DEFAULT 0');
    asset_ensure_column('asset_common_admin_rows', 'scope_office_id', 'INT NOT NULL DEFAULT 0');
    ...
    asset_ensure_text_column('asset_field_options', 'option_value', 'TEXT NOT NULL');
    asset_ensure_text_column('asset_field_options', 'option_label', 'TEXT NOT NULL');
    asset_ensure_segment_indexes();
    asset_ensure_index('asset_filter_items', 'idx_asset_filter_items_segment_field', ['segment_id', 'field_id', 'active_status']);
    asset_ensure_index('asset_filter_items', 'idx_asset_filter_items_segment_key', ['segment_id', 'field_key', 'active_status']);
    asset_drop_index_if_exists('asset_common_profiles', 'uniq_asset_common_profiles_category');
    asset_ensure_unique_index('asset_common_profiles', 'uniq_asset_common_profiles_segment_category', ['segment_id', 'category_id']);
    asset_ensure_index('asset_common_profiles', 'idx_asset_common_profiles_segment', ['segment_id']);
    asset_ensure_index('asset_common_profiles', 'idx_asset_common_profiles_parent', ['parent_segment_id', 'parent_category_id']);
    asset_ensure_index('asset_common_profile_fields', 'idx_asset_common_profile_fields_profile', ['profile_id']);
    asset_ensure_index('asset_common_profile_fields', 'idx_asset_common_profile_fields_parent', ['parent_field_id']);
    asset_ensure_index('asset_common_admin_rows', 'idx_asset_common_admin_rows_profile', ['profile_id']);
    asset_ensure_index('asset_common_admin_rows', 'idx_asset_common_admin_rows_profile_sort', ['profile_id', 'scope_office_type', 'scope_office_id', 'sort_order', 'id']);
    asset_ensure_index('asset_common_admin_row_values', 'idx_asset_common_admin_row_values_row', ['row_id']);
    asset_ensure_index('asset_common_admin_row_values', 'idx_asset_common_admin_row_values_field', ['child_field_id']);
    asset_ensure_index('assets', 'idx_assets_common_profile', ['common_profile_id']);
    asset_ensure_index('assets', 'idx_assets_common_source_asset', ['common_source_asset_id']);
    asset_ensure_index('assets', 'idx_assets_common_profile_row', ['common_profile_id', 'common_row_key', 'office_type', 'office_id', 'active_status', 'deleted_at']);
...
function asset_schema_cache_is_safe_to_trust(): bool
{
    foreach ([
        ['table' => 'segments', 'column' => 'template_source'],
        ['table' => 'asset_fields', 'column' => 'is_common_row_field'],
        ['table' => 'assets', 'column' => 'common_profile_id'],
        ['table' => 'assets', 'column' => 'common_row_key'],
        ['table' => 'asset_common_admin_rows', 'column' => 'scope_office_type'],
        ['table' => 'asset_common_admin_row_values', 'column' => 'row_id'],
    ] as $check) {
        if (!asset_schema_column_exists($check['table'], $check['column'])) {
            return false;
        }
    }
    ...
    foreach ([
        'asset_filter_items',
        'asset_common_profiles',
        'asset_common_profile_fields',
        'asset_common_admin_rows',
        'asset_common_admin_row_values',
        'asset_download_jobs',
        'project_backup_jobs',
    ] as $table) {
        if (!asset_schema_table_exists($table)) {
            return false;
        }
    }
...
function asset_schema_table_exists(string $table): bool
{
...
function asset_schema_column_exists(string $table, string $column): bool
{
...
function asset_ensure_text_column(string $table, string $column, string $definition): void
{
    $stmt = db()->prepare('SELECT DATA_TYPE, COLUMN_TYPE, IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ? LIMIT 1');
    $stmt->execute([$table, $column]);
    $columnInfo = $stmt->fetch(PDO::FETCH_ASSOC) ?: null;
    if (!$columnInfo) {
        return;
    }
    $dataType = strtolower((string)($columnInfo['DATA_TYPE'] ?? ''));
    $columnType = strtolower((string)($columnInfo['COLUMN_TYPE'] ?? ''));
    $isNullable = strtoupper((string)($columnInfo['IS_NULLABLE'] ?? 'YES'));
    if ($dataType === 'text' && $isNullable === 'NO' && str_contains($columnType, 'text')) {
        return;
    }
...
function asset_ensure_segment_indexes(): void
{
    ...
    asset_ensure_index('assets', 'idx_assets_segment_active', ['segment_id', 'active_status', 'deleted_at']);
    asset_ensure_index('assets', 'idx_assets_segment_category_active', ['segment_id', 'category_id', 'active_status', 'deleted_at']);
    asset_ensure_index('assets', 'idx_assets_segment_subcategory_active', ['segment_id', 'subcategory_id', 'active_status', 'deleted_at']);
    asset_ensure_index('assets', 'idx_assets_segment_office_active', ['segment_id', 'office_type', 'office_id', 'active_status', 'deleted_at']);
    ...
    asset_ensure_index('asset_values', 'idx_asset_values_asset_field', ['asset_id', 'field_id']);
    asset_ensure_index('asset_file_values', 'idx_asset_file_values_asset_field', ['asset_id', 'field_id']);
...
function asset_segment_new_batch_options(): array
{
    return [
        0 => 'No batch',
        7 => 'For 7 days',
...
function asset_normalize_segment_new_batch_days(mixed $value): int
{
    $value = (int)$value;
    return array_key_exists($value, asset_segment_new_batch_options()) ? $value : 0;
}

function asset_segment_is_new_batch(array $segment): bool
{
    $days = asset_normalize_segment_new_batch_days($segment['new_batch_days'] ?? 0);
    if ($days === 0) {
        return false;
    }
    if ($days === -1) {
    ...
    $createdAt = trim((string)($segment['created_at'] ?? ''));
    if ($createdAt === '') {
        return false;
    }
    ...
    return $createdTs >= (time() - ($days * 86400));
}
...
function create_asset_segment(string $segmentName, ?int $sortOrder = null, ?int $newBatchDays = 0): int
{
    $newBatchDays = asset_normalize_segment_new_batch_days($newBatchDays);
    ...
    $insert = db()->prepare('INSERT INTO segments (segment_name, active_status, new_batch_days, sort_order, created_at) VALUES (?, 1, ?, ?, NOW())');
    $insert->execute([$segmentName, $newBatchDays, $sortOrder]);
    return (int)db()->lastInsertId();
}

function update_asset_segment(int $segmentId, string $segmentName, ?int $sortOrder = null, ?int $newBatchDays = 0): void
{
    $newBatchDays = asset_normalize_segment_new_batch_days($newBatchDays);
    $segmentName = trim($segmentName);
    ...
    db()->prepare('UPDATE segments SET segment_name = ?, new_batch_days = ?, sort_order = ?, updated_at = NOW() WHERE id = ?')->execute([$segmentName, $newBatchDays, $normalizedSortOrder, $segmentId]);
}
...
function asset_seed_default_fields(): void
{
    ...
    $defaults = [
        [
            ...
            'label' => 'Description / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£',
            'data_type' => 'text',
            'is_required' => 1,
            'sort_order' => 10,
        ],
        [
            ...
            'label' => 'Date of Purchase / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â° ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¤ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¾ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“',
            'data_type' => 'date',
            'is_required' => 0,
            'sort_order' => 20,
        ],
        [
            ...
            'label' => 'Condition / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¥ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¾',
            'data_type' => 'dropdown',
            'is_required' => 1,
            'sort_order' => 30,
            'options' => [
                ['value' => 'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯', 'label' => 'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯'],
                ['value' => 'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯', 'label' => 'ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¹ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯'],
            ],
        ],
        [
            'field_key' => 'remarks',
            'label' => 'Remarks / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â®ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¤ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¯',
            'data_type' => 'text',
            'is_required' => 0,
            'sort_order' => 40,
        ],
...
function asset_common_definition_mode_superadmin_defined(): string
{
    return 'superadmin_defined';
}

function asset_common_definition_mode_options(): array
{
    return [
        asset_common_definition_mode_user_defined() => 'User Defined',
        asset_common_definition_mode_superadmin_defined() => 'Superadmin Defined',
    ];
}

function asset_normalize_common_definition_mode(?string $value): string
{
    $value = trim((string)$value);
    return in_array($value, array_keys(asset_common_definition_mode_options()), true)
        ? $value
        : asset_common_definition_mode_user_defined();
}

function asset_common_row_policy_fixed(): string
{
    return 'fixed';
}

function asset_common_row_policy_addable(): string
{
    return 'addable';
}

function asset_common_row_policy_options(): array
{
    return [
        asset_common_row_policy_fixed() => 'Fixed',
        asset_common_row_policy_addable() => 'Addable',
    ];
}

function asset_normalize_common_row_policy(?string $value): string
{
    $value = trim((string)$value);
    return in_array($value, array_keys(asset_common_row_policy_options()), true)
        ? $value
        : asset_common_row_policy_fixed();
}

function asset_common_source_type_normal(): string
{
    return 'normal';
}

function asset_common_source_type_user_defined(): string
{
    return 'user_defined_common';
}

function asset_common_source_type_superadmin_defined(): string
{
    return 'superadmin_defined_common';
}
...
{
    if (array_key_exists('common_row_config_present', $input)) {
        return !empty($input['common_row_config_present']);
    }
    foreach ([
        'is_common_row_field',
        'common_profile_category_id',
        'common_definition_mode',
        'common_row_policy',
        'common_parent_segment_id',
        'common_parent_category_id',
        'common_parent_field_id',
    ] as $key) {
...
function get_asset_common_profile(int $profileId, bool $includeInactive = false): ?array
{
    $sql = 'SELECT * FROM asset_common_profiles WHERE id = ?';
    if (!$includeInactive) {
        $sql .= ' AND active_status = 1';
    }
    $sql .= ' LIMIT 1';
...
function get_asset_common_profile_by_category(int $categoryId, ?int $segmentId = null, bool $includeInactive = false): ?array
{
    $sql = 'SELECT * FROM asset_common_profiles WHERE category_id = ?';
    $params = [$categoryId];
    $normalizedSegmentId = asset_normalize_segment_id($segmentId);
    if ($normalizedSegmentId > 0) {
        $sql .= ' AND segment_id = ?';
...
{
    $stmt = db()->prepare('
        SELECT pf.*, p.segment_id, p.category_id, p.definition_mode, p.row_policy, p.parent_segment_id, p.parent_category_id, p.active_status AS profile_active_status
        FROM asset_common_profile_fields pf
        JOIN asset_common_profiles p ON p.id = pf.profile_id
        WHERE pf.child_field_id = ?
        LIMIT 1
    ');
    $stmt->execute([$childFieldId]);
    $row = $stmt->fetch();
    return $row ?: null;
}

function get_asset_common_profile_field_bindings_for_segment(int $segmentId): array
{
    $stmt = db()->prepare(
        'SELECT pf.*, p.segment_id, p.category_id, p.definition_mode, p.row_policy, p.parent_segment_id, p.parent_category_id,
                child.field_key AS child_field_key, child.label AS child_field_label,
                parent.field_key AS parent_field_key, parent.label AS parent_field_label
         FROM asset_common_profile_fields pf
         JOIN asset_common_profiles p ON p.id = pf.profile_id
...
{
    $map = [];
    foreach (get_asset_categories(true, $segmentId) as $category) {
        $profile = get_asset_common_profile_by_category((int)$category['id'], $segmentId, $includeInactive);
        if ($profile) {
            $map[(int)$category['id']] = $profile;
        }
    }
    return $map;
}

function asset_segment_category_count(int $segmentId, bool $includeInactive = true): int
{
    return count(get_asset_categories($includeInactive, $segmentId));
}

function asset_segment_common_rows_supported(int $segmentId): bool
{
    return asset_segment_category_count($segmentId, true) <= 1;
}

function asset_segment_common_profile_category_id(int $segmentId): int
{
    $categories = get_asset_categories(true, $segmentId);
...
function asset_segment_has_common_row_configuration(int $segmentId): bool
{
    if ($segmentId <= 0) {
        return false;
    }
    $stmt = db()->prepare('SELECT COUNT(*) FROM asset_common_profiles WHERE segment_id = ?');
    $stmt->execute([$segmentId]);
    if ((int)$stmt->fetchColumn() > 0) {
        return true;
    }
    $stmt = db()->prepare('SELECT COUNT(*) FROM asset_fields WHERE segment_id = ? AND is_common_row_field = 1');
    $stmt->execute([$segmentId]);
    return (int)$stmt->fetchColumn() > 0;
}

function asset_common_profile_allows_manual_rows(?array $profile): bool
{
    if (!$profile) {
        return true;
    }
    return (string)($profile['row_policy'] ?? '') === asset_common_row_policy_addable();
}

function asset_common_category_allows_manual_rows(int $segmentId, int $categoryId): bool
{
    if ($segmentId <= 0 || $categoryId <= 0) {
        return true;
    }
    return asset_common_profile_allows_manual_rows(
        get_asset_common_profile_by_category($categoryId, $segmentId, true)
    );
}

function asset_common_profile_id_for_category(int $segmentId, int $categoryId): ?int
{
    if ($segmentId <= 0 || $categoryId <= 0) {
        return null;
    }
    $profile = get_asset_common_profile_by_category($categoryId, $segmentId, true);
    $profileId = (int)($profile['id'] ?? 0);
    return $profileId > 0 ? $profileId : null;
}

function asset_common_profile_child_field_keys(int $profileId): array
{
    if ($profileId <= 0) {
        return [];
    }
    $keys = [];
    foreach (get_asset_common_profile_fields($profileId) as $binding) {
        $fieldKey = trim((string)($binding['child_field_key'] ?? ''));
        if ($fieldKey !== '') {
            $keys[$fieldKey] = true;
        }
    }
...
function asset_common_asset_is_generated(array $asset): bool
{
    return (int)($asset['common_profile_id'] ?? 0) > 0
        && (int)($asset['is_user_added_row'] ?? 1) === 0;
}

function asset_common_asset_is_manual_common(array $asset): bool
{
    return (int)($asset['common_profile_id'] ?? 0) > 0
        && (int)($asset['is_user_added_row'] ?? 1) === 1;
}

function asset_common_locked_field_keys_for_asset(array $asset): array
{
    if (!asset_common_asset_is_generated($asset)) {
        return [];
    }
    return asset_common_profile_child_field_keys((int)($asset['common_profile_id'] ?? 0));
}

function asset_common_current_normalized_value(array $field, array $asset): array
{
    $type = (string)($field['data_type'] ?? 'text');
    $fieldKey = (string)($field['field_key'] ?? '');
    $displayValue = $asset['values'][$fieldKey] ?? null;
    $normalized = [
        'value_text' => null,
        'value_number' => null,
        'value_date' => null,
    ...
    return match ($type) {
        'number' => [
            'value_text' => null,
            'value_number' => asset_normalize_number_string((string)$displayValue),
            'value_date' => null,
            'value_bool' => null,
            'value_option' => null,
            'display' => asset_normalize_number_string((string)$displayValue) ?? '',
        ],
        'date' => [
            'value_text' => null,
            'value_number' => null,
            'value_date' => (string)$displayValue,
            'value_bool' => null,
            'value_option' => null,
        ...
        'yes_no' => [
            ...
            'value_number' => null,
            'value_date' => null,
            'value_bool' => asset_normalize_yes_no((string)$displayValue),
            'value_option' => null,
            'display' => (string)$displayValue,
        ],
        'dropdown', 'conditional' => [
            ...
            'value_date' => null,
            'value_bool' => null,
            'value_option' => (string)$displayValue,
            'display' => (string)$displayValue,
        ],
        default => [
            'value_text' => (string)$displayValue,
            'value_number' => null,
            'value_date' => null,
            'value_bool' => null,
            'value_option' => null,
...
function asset_common_parent_field_candidates_by_segment(bool $includeInactive = false): array
{
    $segments = get_asset_segments(true);
    $supportedTypes = array_fill_keys(asset_common_supported_field_data_types(), true);
    $catalog = [];
    foreach ($segments as $segment) {
        $segmentId = (int)($segment['id'] ?? 0);
        $catalog[$segmentId] = array_values(array_filter(
            get_asset_fields($includeInactive, $segmentId),
            static function (array $field) use ($supportedTypes): bool {
                $type = (string)($field['data_type'] ?? '');
                return !asset_is_conditional_secondary($field)
                    && isset($supportedTypes[$type]);
            }
        ));
    }
    return $catalog;
...
function asset_common_remove_generated_assets_for_profile(int $profileId, ?int $actingUserId = null): void
{
    if ($profileId <= 0) {
        return;
    }
    $actorUserId = $actingUserId !== null && $actingUserId > 0 ? $actingUserId : ((int)(current_user()['id'] ?? 0) ?: 1);
    $stmt = db()->prepare(
        'SELECT id
         FROM assets
         WHERE common_profile_id = ?
           AND deleted_at IS NULL
           AND active_status = 1'
    );
    $stmt->execute([$profileId]);
    foreach ($stmt->fetchAll(PDO::FETCH_COLUMN) as $assetId) {
        $assetId = (int)$assetId;
        if ($assetId <= 0) {
            continue;
        }
        $beforeAsset = get_asset($assetId, true);
        if (!$beforeAsset || !empty($beforeAsset['deleted_at'])) {
            continue;
        }
        db()->prepare('UPDATE assets SET active_status = 0, deleted_at = NOW(), updated_by = ?, updated_at = NOW() WHERE id = ?')->execute([$actorUserId, $assetId]);
        asset_filter_index_sync_asset_change($beforeAsset, null);
        add_asset_activity_log_with_segment(
            (int)($beforeAsset['segment_id'] ?? asset_default_segment_id()),
            $assetId,
            $actorUserId,
            'common_row_removed',
            'Common row removed after profile change.',
            asset_common_activity_details($beforeAsset)
        );
    }
}

function asset_delete_common_profile_if_empty(int $profileId, ?int $actingUserId = null): void
{
    $countStmt = db()->prepare('SELECT COUNT(*) FROM asset_common_profile_fields WHERE profile_id = ?');
    $countStmt->execute([$profileId]);
    if ((int)$countStmt->fetchColumn() > 0) {
        return;
    }
    $rowStmt = db()->prepare('SELECT id FROM asset_common_admin_rows WHERE profile_id = ?');
    $rowStmt->execute([$profileId]);
    $rowIds = array_map('intval', $rowStmt->fetchAll(PDO::FETCH_COLUMN) ?: []);
    if ($rowIds) {
        $placeholders = implode(',', array_fill(0, count($rowIds), '?'));
        db()->prepare("DELETE FROM asset_common_admin_row_values WHERE row_id IN ({$placeholders})")->execute($rowIds);
    }
    asset_common_remove_generated_assets_for_profile($profileId, $actingUserId);
    db()->prepare('DELETE FROM asset_common_admin_rows WHERE profile_id = ?')->execute([$profileId]);
    db()->prepare('DELETE FROM asset_common_profiles WHERE id = ?')->execute([$profileId]);
}

function asset_upsert_common_profile_binding(int $childFieldId, array $payload): array
{
    if (empty($payload['common_row_config_present'])) {
    ...
    $profileCategoryId = (int)($payload['common_profile_category_id'] ?? 0);
    $definitionMode = asset_normalize_common_definition_mode((string)($payload['common_definition_mode'] ?? ''));
    $rowPolicy = asset_normalize_common_row_policy((string)($payload['common_row_policy'] ?? ''));
    $parentSegmentId = $definitionMode === asset_common_definition_mode_user_defined()
        ? asset_normalize_segment_id((int)($payload['common_parent_segment_id'] ?? 0))
        : null;
    $parentCategoryId = $definitionMode === asset_common_definition_mode_user_defined()
        ? (int)($payload['common_parent_category_id'] ?? 0)
        : null;
    $parentFieldId = $definitionMode === asset_common_definition_mode_user_defined()
    ...
    if (!$profile) {
        $stmt = db()->prepare('
            INSERT INTO asset_common_profiles (
                segment_id, category_id, definition_mode, row_policy, parent_segment_id, parent_category_id, active_status, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, 1, NOW())
        ');
        $stmt->execute([
            $segmentId,
        ...
        db()->prepare('
            UPDATE asset_common_profiles
            SET definition_mode = ?, row_policy = ?, parent_segment_id = ?, parent_category_id = ?, active_status = 1, updated_at = NOW()
            WHERE id = ?
        ')->execute([
            $definitionMode,
            $rowPolicy,
...
function asset_sync_common_profiles_after_binding_change(array $bindingChange, ?int $actingUserId = null): void
{
    $oldProfileId = (int)($bindingChange['old_profile_id'] ?? 0);
    $newProfileId = (int)($bindingChange['new_profile_id'] ?? 0);
    if ($oldProfileId > 0 && $oldProfileId !== $newProfileId && get_asset_common_profile($oldProfileId, true)) {
        asset_sync_common_rows_for_profile_across_offices($oldProfileId, $actingUserId);
    }
    if ($newProfileId > 0) {
...
function asset_sync_user_defined_child_fields_from_parent(int $parentFieldId, ?int $segmentId = null): void
{
    ...
    foreach (asset_common_user_defined_bindings_by_parent_field($parentFieldId) as $binding) {
        ...
        $childIsConditional = asset_is_conditional_primary($childField);
        if ($parentIsConditional !== $childIsConditional) {
            continue;
        }
...
function get_asset_common_profiles_for_segment(int $segmentId, bool $includeInactive = false): array
{
    $sql = 'SELECT * FROM asset_common_profiles WHERE segment_id = ?';
    $params = [$segmentId];
    if (!$includeInactive) {
        $sql .= ' AND active_status = 1';
    }
...
function get_asset_common_profiles_by_parent(int $parentSegmentId, int $parentCategoryId, bool $includeInactive = false): array
{
    $sql = 'SELECT * FROM asset_common_profiles WHERE parent_segment_id = ? AND parent_category_id = ?';
    $params = [$parentSegmentId, $parentCategoryId];
    if (!$includeInactive) {
        $sql .= ' AND active_status = 1';
    }
    $sql .= ' ORDER BY id ASC';
...
function asset_common_scope_token(int $officeType, int $officeId): string
{
    return $officeType . ':' . $officeId;
}

function asset_common_parse_scope_token(string $token): array
{
    $token = trim($token);
    if ($token === '' || strtolower($token) === 'global' || $token === '0:0') {
        return ['office_type' => 0, 'office_id' => 0];
    }
    $parts = explode(':', $token, 2);
    $officeType = (int)($parts[0] ?? 0);
    ...
    return ['office_type' => $officeType, 'office_id' => $officeId];
}

function asset_common_scope_label(int $officeType, int $officeId): string
{
    if ($officeType <= 0 && $officeId <= 0) {
        return 'All Offices (Global)';
    }
    if ($officeId === 0) {
        return match ($officeType) {
            ...
            4 => 'All Division',
            5 => 'All Subdivision',
            default => 'All Offices (Global)',
        };
    }
    return office_kind_to_type(office_type_to_kind($officeType)) > 0
        ? ucfirst(str_replace('_', '-', office_type_to_kind($officeType))) . ' - ' . office_name_from_type_id($officeType, $officeId)
        : office_name_from_type_id($officeType, $officeId);
}

function asset_common_scope_options(): array
{
    $options = [
        asset_common_scope_token(0, 0) => 'All Offices (Global)',
        asset_common_scope_token(2, 0) => 'All Zone',
        asset_common_scope_token(3, 0) => 'All Circle',
        asset_common_scope_token(4, 0) => 'All Division',
        asset_common_scope_token(5, 0) => 'All Subdivision',
    ];
    $overview = get_offices_overview();
    foreach ($overview['zones'] ?? [] as $row) {
        $options[asset_common_scope_token(2, (int)$row['id'])] = 'Zone - ' . (string)$row['office_name'];
    }
    foreach ($overview['circles'] ?? [] as $row) {
        $options[asset_common_scope_token(3, (int)$row['id'])] = 'Circle - ' . (string)$row['office_name'];
    }
    foreach ($overview['divisions'] ?? [] as $row) {
        $options[asset_common_scope_token(4, (int)$row['id'])] = 'Division - ' . (string)$row['office_name'];
    }
    foreach ($overview['subdivisions'] ?? [] as $row) {
        $options[asset_common_scope_token(5, (int)$row['id'])] = 'Sub-division - ' . (string)$row['office_name'];
    }
    return $options;
}

function asset_common_sync_actor_user_id(?int $preferredUserId = null, ?array $contextAsset = null): int
{
    $preferredUserId = (int)$preferredUserId;
    if ($preferredUserId > 0) {
        return $preferredUserId;
    }
    if ($contextAsset) {
        $updatedBy = (int)($contextAsset['updated_by'] ?? 0);
        if ($updatedBy > 0) {
            return $updatedBy;
        }
        $createdBy = (int)($contextAsset['created_by'] ?? 0);
        if ($createdBy > 0) {
            return $createdBy;
        }
    }
    $currentUser = current_user();
    if ($currentUser && (int)($currentUser['id'] ?? 0) > 0) {
        return (int)$currentUser['id'];
    }
    $stmt = db()->query('SELECT id FROM users WHERE active_status = 1 ORDER BY office_role DESC, id ASC LIMIT 1');
    return (int)($stmt->fetchColumn() ?: 1);
}

function asset_common_generated_row_key_for_source_asset(int $sourceAssetId): string
{
    return 'source_asset_' . $sourceAssetId;
}

function asset_common_activity_details(array $asset): array
{
    $details = [];
    $sourceType = (string)($asset['common_source_type'] ?? '');
    if ($sourceType !== '') {
        $details[] = ['field' => 'Source Type', 'value' => $sourceType];
    }
    if ((int)($asset['common_source_asset_id'] ?? 0) > 0) {
        $details[] = ['field' => 'Source Asset ID', 'value' => (string)(int)$asset['common_source_asset_id']];
    }
    if ((string)($asset['common_row_key'] ?? '') !== '') {
        $details[] = ['field' => 'Common Row Key', 'value' => (string)$asset['common_row_key']];
    }
    return $details;
}

function asset_common_activity_signature(?array $asset): string
{
    if (!$asset) {
        return '';
    }
    $signature = [
        ...
        'subcategory_id' => (int)($asset['subcategory_id'] ?? 0),
        'common_row_key' => (string)($asset['common_row_key'] ?? ''),
        'common_source_type' => (string)($asset['common_source_type'] ?? ''),
        'common_source_asset_id' => (int)($asset['common_source_asset_id'] ?? 0),
        'values' => (array)($asset['values'] ?? []),
    ];
    return json_encode($signature, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?: '';
}

function asset_common_fetch_source_assets(int $segmentId, int $categoryId, int $officeType, int $officeId): array
{
    if ($segmentId <= 0 || $categoryId <= 0 || $officeType <= 0 || $officeId <= 0) {
        return [];
    }
    $stmt = db()->prepare(
        'SELECT a.*, c.name AS category_name, s.name AS subcategory_name, creator.email_id AS created_by_email, editor.email_id AS updated_by_email
         FROM assets a
    ...
    $stmt->execute([$segmentId, $categoryId, $officeType, $officeId]);
    $rows = $stmt->fetchAll();
    $assetIds = array_map(static fn(array $row): int => (int)$row['id'], $rows);
...
function asset_common_fetch_admin_rows(int $profileId, ?int $officeType = null, ?int $officeId = null): array
{
    $params = [$profileId];
    $scopeClause = '';
    if (($officeType ?? 0) > 0) {
        $scopePairs = [[0, 0], [(int)$officeType, 0]];
        if (($officeId ?? 0) > 0) {
            $scopePairs[] = [(int)$officeType, (int)$officeId];
        }
        $conditions = [];
        foreach ($scopePairs as [$scopeType, $scopeId]) {
            $conditions[] = '(r.scope_office_type = ? AND r.scope_office_id = ?)';
            $params[] = $scopeType;
            $params[] = $scopeId;
        }
    ...
    $stmt = db()->prepare(
        'SELECT r.id, r.profile_id, r.stable_row_key, r.scope_office_type, r.scope_office_id, r.sort_order, r.active_status,
                v.child_field_id, v.value_text
         FROM asset_common_admin_rows r
         LEFT JOIN asset_common_admin_row_values v ON v.row_id = r.id
         WHERE r.profile_id = ?
           AND r.active_status = 1' . $scopeClause . '
    ...
    foreach ($stmt->fetchAll() as $row) {
        ...
        if (!isset($rows[$rowId])) {
            $rows[$rowId] = [
                'id' => $rowId,
                'stable_row_key' => (string)($row['stable_row_key'] ?? ''),
                'scope_office_type' => (int)($row['scope_office_type'] ?? 0),
                'scope_office_id' => (int)($row['scope_office_id'] ?? 0),
                'scope_token' => asset_common_scope_token((int)($row['scope_office_type'] ?? 0), (int)($row['scope_office_id'] ?? 0)),
                'scope_label' => asset_common_scope_label((int)($row['scope_office_type'] ?? 0), (int)($row['scope_office_id'] ?? 0)),
                'sort_order' => (int)($row['sort_order'] ?? 0),
                'values' => [],
            ];
        }
        $childFieldId = (int)($row['child_field_id'] ?? 0);
        if ($childFieldId > 0) {
            $rows[$rowId]['values'][$childFieldId] = (string)($row['value_text'] ?? '');
        }
    }
    return array_values($rows);
}

function get_asset_common_admin_rows_detailed(int $profileId): array
{
    return asset_common_fetch_admin_rows($profileId);
}

function asset_common_specific_office_scope_options(): array
{
    $options = [];
    $overview = get_offices_overview();
    foreach (['zones' => 2, 'circles' => 3, 'divisions' => 4, 'subdivisions' => 5] as $key => $officeType) {
        foreach ($overview[$key] ?? [] as $row) {
            $officeId = (int)($row['id'] ?? 0);
            if ($officeId <= 0) {
                continue;
            }
            $options[asset_common_scope_token($officeType, $officeId)] = asset_common_scope_label($officeType, $officeId);
        }
    }
    return $options;
}

function asset_common_superadmin_segments(): array
{
    $segments = [];
    foreach (get_asset_segments(true) as $segment) {
        ...
        $profiles = get_asset_common_profiles_for_segment($segmentId, true);
        foreach ($profiles as $profile) {
            if ((string)($profile['definition_mode'] ?? '') === asset_common_definition_mode_superadmin_defined()) {
                $segments[] = $segment;
                break;
            }
        }
...
function asset_common_superadmin_profiles_by_category(int $segmentId): array
{
    $profiles = [];
    foreach (get_asset_common_profiles_for_segment($segmentId, true) as $profile) {
        if ((string)($profile['definition_mode'] ?? '') !== asset_common_definition_mode_superadmin_defined()) {
            continue;
        }
        $profiles[(int)($profile['category_id'] ?? 0)] = $profile;
    }
    return $profiles;
}

function asset_common_all_category_key(): string
{
    return '__all__';
}

function asset_common_superadmin_profiles_for_segment(int $segmentId): array
{
    return array_values(asset_common_superadmin_profiles_by_category($segmentId));
}

function asset_common_all_category_field_definitions(int $segmentId): array
{
    $definitions = [];
    foreach (asset_common_superadmin_profiles_for_segment($segmentId) as $profile) {
        foreach (get_asset_common_profile_fields((int)($profile['id'] ?? 0)) as $profileField) {
            $childFieldId = (int)($profileField['child_field_id'] ?? 0);
            if ($childFieldId <= 0 || isset($definitions[$childFieldId])) {
                continue;
            }
            $field = get_asset_field($childFieldId, $segmentId);
            if (!$field) {
            ...
            $definitions[$childFieldId] = [
                'child_field_id' => $childFieldId,
                'child_field_label' => (string)($profileField['child_field_label'] ?? ($field['label'] ?? 'Field')),
                'field_sort_order' => (int)($field['sort_order'] ?? 0),
            ];
        }
    }
    uasort($definitions, static function (array $left, array $right): int {
        $sortCompare = ((int)$left['field_sort_order']) <=> ((int)$right['field_sort_order']);
        if ($sortCompare !== 0) {
            return $sortCompare;
        }
        return strcasecmp((string)$left['child_field_label'], (string)$right['child_field_label']);
    });
    return array_values($definitions);
}

function asset_common_all_category_rows(int $segmentId): array
{
    $rows = [];
    foreach (asset_common_superadmin_profiles_for_segment($segmentId) as $profile) {
        foreach (get_asset_common_admin_rows_detailed((int)($profile['id'] ?? 0)) as $row) {
            $stableRowKey = (string)($row['stable_row_key'] ?? '');
            $scopeToken = (string)($row['scope_token'] ?? asset_common_scope_token(
                (int)($row['scope_office_type'] ?? 0),
                (int)($row['scope_office_id'] ?? 0)
            ));
            $mergeKey = $stableRowKey !== ''
                ? $stableRowKey . '|' . $scopeToken . '|' . (int)($row['sort_order'] ?? 0)
                : 'row_' . (int)($row['id'] ?? 0);
            if (!isset($rows[$mergeKey])) {
                $rows[$mergeKey] = [
                    'stable_row_key' => $stableRowKey,
                    'scope_office_type' => (int)($row['scope_office_type'] ?? 0),
                    'scope_office_id' => (int)($row['scope_office_id'] ?? 0),
            ...
            foreach ((array)($row['values'] ?? []) as $childFieldId => $valueText) {
                $childFieldId = (int)$childFieldId;
                if ($childFieldId <= 0) {
                    continue;
                }
                $rows[$mergeKey]['values'][$childFieldId] = (string)$valueText;
            }
        }
    }
    uasort($rows, static function (array $left, array $right): int {
        ...
        return strcmp((string)$left['stable_row_key'], (string)$right['stable_row_key']);
    });
    return array_values($rows);
}

function asset_common_profile_resolve_for_segment_category(int $segmentId, string $commonCategory): ?array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    if ($segmentId <= 0) {
        return null;
    }
    $categoryId = (int)$commonCategory;
    $profile = get_asset_common_profile_by_category($categoryId, $segmentId, true);
    if (!$profile || (string)($profile['definition_mode'] ?? '') !== asset_common_definition_mode_superadmin_defined()) {
        return null;
    }
    return $profile;
}

function asset_common_sync_target_offices_for_profile(array $profile): array
{
    $definitionMode = (string)($profile['definition_mode'] ?? '');
    if ($definitionMode === asset_common_definition_mode_superadmin_defined()) {
        $overview = get_offices_overview();
        $scopes = [];
        foreach (['zones' => 2, 'circles' => 3, 'divisions' => 4, 'subdivisions' => 5] as $key => $officeType) {
            foreach ($overview[$key] ?? [] as $row) {
                ...
                $scopes[$officeType . ':' . $officeId] = ['office_type' => $officeType, 'office_id' => $officeId];
            }
        }
    ...
    $stmt = db()->prepare(
        'SELECT DISTINCT office_type, office_id
         FROM assets
         WHERE (
                (segment_id = ? AND category_id = ?)
                OR (common_profile_id = ?)
               )
           AND deleted_at IS NULL
           AND active_status = 1
           AND office_id > 0
           AND office_type > 0
         ORDER BY office_type ASC, office_id ASC'
    );
    $stmt->execute([
        (int)($profile['parent_segment_id'] ?? 0),
        (int)($profile['parent_category_id'] ?? 0),
        (int)($profile['id'] ?? 0),
    ]);
    return array_map(
        static fn(array $scope): array => [
            'office_type' => (int)($scope['office_type'] ?? 0),
            'office_id' => (int)($scope['office_id'] ?? 0),
        ],
        $stmt->fetchAll()
    );
}

function asset_common_admin_row_key(): string
{
    return 'admin_row_' . bin2hex(random_bytes(8));
}

function asset_common_admin_row_payload_values(array $profileFields, array $input): array
{
    $childFieldMap = asset_common_child_field_map((int)($profileFields[0]['segment_id'] ?? 0));
    $fieldMapByKey = [];
    foreach ($childFieldMap as $field) {
        $fieldMapByKey[(string)$field['field_key']] = $field;
    }
    $normalizedValues = [];
    foreach ($profileFields as $profileField) {
        $childFieldId = (int)($profileField['child_field_id'] ?? 0);
        if ($childFieldId <= 0 || !isset($childFieldMap[$childFieldId])) {
            continue;
        }
        $childField = $childFieldMap[$childFieldId];
        $rawValue = $input[$childFieldId] ?? null;
        $normalized = asset_common_normalize_child_field_value($childField, $rawValue, $fieldMapByKey);
        $normalizedValues[$childFieldId] = (string)($normalized['display'] ?? '');
    }
    return $normalizedValues;
}

function save_asset_common_admin_row(int $profileId, array $input, ?int $rowId = null): void
{
    $profile = get_asset_common_profile($profileId, true);
    if (!$profile) {
        throw new RuntimeException('Common-row profile not found.');
    }
    if ((string)($profile['definition_mode'] ?? '') !== asset_common_definition_mode_superadmin_defined()) {
        throw new RuntimeException('This profile is not superadmin-defined.');
    }
    $profileFields = get_asset_common_profile_fields($profileId);
    if ($profileFields === []) {
        throw new RuntimeException('No bound fields are configured for this profile.');
    }
    $scope = asset_common_parse_scope_token((string)($input['scope_token'] ?? '0:0'));
    $sortOrder = max(1, (int)($input['sort_order'] ?? next_sort_order_for_filters('asset_common_admin_rows', ['profile_id' => $profileId])));
    $segmentFieldMapById = [];
    $segmentFieldMapByKey = [];
    foreach (get_asset_fields(true, (int)$profile['segment_id']) as $fieldRow) {
        $segmentFieldMapById[(int)$fieldRow['id']] = $fieldRow;
    ...
    try {
        if ($rowId !== null && $rowId > 0) {
            $stmt = db()->prepare('SELECT id FROM asset_common_admin_rows WHERE id = ? AND profile_id = ? LIMIT 1');
            $stmt->execute([$rowId, $profileId]);
            if (!(int)$stmt->fetchColumn()) {
                throw new RuntimeException('Admin row not found.');
            }
            db()->prepare('UPDATE asset_common_admin_rows SET scope_office_type = ?, scope_office_id = ?, sort_order = ?, active_status = 1, updated_at = NOW() WHERE id = ?')->execute([$scope['office_type'], $scope['office_id'], $sortOrder, $rowId]);
        } else {
            db()->prepare('INSERT INTO asset_common_admin_rows (profile_id, stable_row_key, scope_office_type, scope_office_id, sort_order, active_status, created_at) VALUES (?, ?, ?, ?, ?, 1, NOW())')->execute([
                $profileId,
                asset_common_admin_row_key(),
                $scope['office_type'],
                $scope['office_id'],
                $sortOrder,
            ]);
        ...
        foreach ($values as $childFieldId => $valueText) {
            $existsStmt = db()->prepare('SELECT id FROM asset_common_admin_row_values WHERE row_id = ? AND child_field_id = ? LIMIT 1');
            $existsStmt->execute([$rowId, $childFieldId]);
            $existingId = (int)($existsStmt->fetchColumn() ?: 0);
            if ($existingId > 0) {
                db()->prepare('UPDATE asset_common_admin_row_values SET value_text = ?, updated_at = NOW() WHERE id = ?')->execute([$valueText, $existingId]);
            } else {
                db()->prepare('INSERT INTO asset_common_admin_row_values (row_id, child_field_id, value_text, created_at) VALUES (?, ?, ?, NOW())')->execute([$rowId, $childFieldId, $valueText]);
            }
        }
        db()->commit();
        ...
        foreach ($createdTutorialFiles as $createdTutorialFile) {
            if ($createdTutorialFile !== '' && is_file($createdTutorialFile)) {
                @unlink($createdTutorialFile);
            }
        }
        throw $e;
    }
...
function delete_asset_common_admin_row(int $profileId, int $rowId): void
{
    $stmt = db()->prepare('SELECT id FROM asset_common_admin_rows WHERE id = ? AND profile_id = ? LIMIT 1');
    $stmt->execute([$rowId, $profileId]);
    if (!(int)$stmt->fetchColumn()) {
    ...
    db()->beginTransaction();
    try {
        db()->prepare('DELETE FROM asset_common_admin_row_values WHERE row_id = ?')->execute([$rowId]);
        db()->prepare('DELETE FROM asset_common_admin_rows WHERE id = ?')->execute([$rowId]);
        db()->commit();
        asset_sync_common_rows_for_profile_across_offices($profileId);
    } catch (Throwable $e) {
        if (db()->inTransaction()) {
...
function asset_common_admin_template_column_definitions(int $profileId): array
{
    $profile = get_asset_common_profile($profileId, true);
    if (!$profile) {
        throw new RuntimeException('Common-row profile not found.');
    ...
    $columns = [[
        'key' => 'office_scope',
        'label' => 'Office Name',
        'instruction_label' => 'Office Name',
        'data_type' => 'dropdown',
        'validation_kind' => 'office_scope',
        'required_input' => true,
...
function build_asset_common_admin_template_workbook(int $profileId): \PhpOffice\PhpSpreadsheet\Spreadsheet
{
    ...
    $profileFields = get_asset_common_profile_fields($profileId);
    $inputColumns = asset_common_admin_template_column_definitions($profileId);
    ...
    $existingRows = get_asset_common_admin_rows_detailed($profileId);
    $existingRowCount = count($existingRows);
    $scopeOptions = array_values(asset_common_scope_options());
    $rowLimit = asset_template_row_limit();
    $startRow = 2;
    $endRow = $startRow + $rowLimit - 1;
    $firstEditableRow = min($endRow + 1, $startRow + $existingRowCount);

    $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet();
    export_apply_spreadsheet_font($spreadsheet);
    $sheet = $spreadsheet->getActiveSheet();
    $sheet->setTitle('data');
    $dropdownSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'dropdowns');
    ...
    $writeListRange = static function (\PhpOffice\PhpSpreadsheet\Spreadsheet $book, \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $listSheet, string $rangeName, array $values, int &$columnIndex) use (&$definedRanges): string {
        ...
        $listSheet->setCellValue($col . '1', $rangeName);
        foreach ($values as $offset => $value) {
            $listSheet->setCellValueExplicit($col . ($offset + 2), $value, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        }
        $rangeRef = '$' . $col . '$2:$' . $col . '$' . (count($values) + 1);
        if (!isset($definedRanges[$rangeName])) {
            $book->addNamedRange(new \PhpOffice\PhpSpreadsheet\NamedRange($rangeName, $listSheet, $rangeRef));
    ...
    foreach ($inputColumns as &$column) {
        if (($column['validation_kind'] ?? '') === 'office_scope') {
            $column['range_name'] = $writeListRange($spreadsheet, $dropdownSheet, 'COMMON_SCOPE_OPTIONS', $scopeOptions, $helperColumnIndex);
        } elseif (($column['validation_kind'] ?? '') === 'dropdown') {
            $column['range_name'] = $writeListRange(
                $spreadsheet,
                $dropdownSheet,
                'COMMON_LIST_' . asset_template_named_token((string)$column['key']),
                $column['options'] ?? [],
                $helperColumnIndex
            );
            ...
            $column['range_name'] = $writeListRange(
                $spreadsheet,
                $dropdownSheet,
                'COMMON_YESNO_' . asset_template_named_token((string)$column['key']),
    ...
    foreach ($displayColumns as $index => $column) {
        ...
        $sheet->setCellValue($letter . '1', asset_template_build_header_rich_text($column));
        $sheet->getColumnDimension($letter)->setWidth(asset_template_column_width($column));
    }
    ...
    foreach ($existingRows as $row) {
        ...
        $sheet->setCellValueExplicit(
            $columnLetters['office_scope'] . $rowNumber,
            (string)($row['scope_label'] ?? asset_common_scope_label(0, 0)),
            \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING
        );
        foreach ($profileFields as $profileField) {
            $childFieldId = (int)($profileField['child_field_id'] ?? 0);
            $columnKey = 'field_' . $childFieldId;
            if (!isset($columnLetters[$columnKey])) {
                continue;
            }
            $sheet->setCellValueExplicit(
                $columnLetters[$columnKey] . $rowNumber,
                (string)($row['values'][$childFieldId] ?? ''),
                \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING
            );
        }
        $rowNumber++;
    ...
    $inputFirstLetter = $columnLetters[$inputColumns[0]['key'] ?? 'office_scope'] ?? 'B';
    $inputLastLetter = $columnLetters[$inputColumns[count($inputColumns) - 1]['key'] ?? 'office_scope'] ?? $inputFirstLetter;
    for ($row = $startRow; $row <= $endRow; $row++) {
        ...
        $instructionFormula = build_asset_template_instruction_formula(
            $inputFirstLetter . $row . ':' . $inputLastLetter . $row,
            $rowRequiredRefs,
            $columnLetters['instruction'] . $row
        );
        $sheet->setCellValue($columnLetters['instruction'] . $row, $instructionFormula);
    }
    ...
    $sheet->getStyle('A1:' . $lastColumnLetter . $endRow)->applyFromArray([
        'borders' => [
            'allBorders' => [
                'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                'color' => ['rgb' => 'D0D4D9'],
    ...
    $sheet->getStyle('A2:' . $lastColumnLetter . $endRow)->getFont()->setSize(10);
    $sheet->getStyle($columnLetters['instruction'] . '2:' . $columnLetters['instruction'] . $endRow)->getAlignment()->setWrapText(true);
    $sheet->getStyle('A1:' . $lastColumnLetter . $endRow)
        ->getProtection()
    ...
    foreach ($inputColumns as $column) {
        ...
        if ((string)$column['data_type'] === 'date') {
            $sheet->getStyle($colLetter . $startRow . ':' . $colLetter . $endRow)->getNumberFormat()->setFormatCode('@');
        } elseif ((string)$column['data_type'] === 'number' && trim((string)($column['number_format_rule'] ?? '')) !== '') {
            $sheet->getStyle($colLetter . $startRow . ':' . $colLetter . $endRow)->getNumberFormat()->setFormatCode('@');
        } elseif ((string)$column['data_type'] === 'number') {
            $sheet->getStyle($colLetter . $startRow . ':' . $colLetter . $endRow)->getNumberFormat()->setFormatCode('0.############');
        }
            ...
            switch ((string)($column['validation_kind'] ?? '')) {
                ...
                default:
                    if ((int)($column['text_max_length'] ?? 0) > 0) {
                        ...
                        $validation->setFormula1((string)(int)$column['text_max_length']);
                        $validation->setError('Keep within ' . (int)$column['text_max_length'] . ' characters.');
                    }
                    break;
            }
        }
...
function output_asset_common_admin_template_download(int $profileId): void
{
    if ($profileId <= 0) {
        throw new RuntimeException('Common-row profile not found.');
    }
    $workbook = build_asset_common_admin_template_workbook($profileId);
    $profile = get_asset_common_profile($profileId, true);
    if (!$profile) {
        throw new RuntimeException('Common-row profile not found.');
    }
    $filename = 'common_rows_profile_' . $profileId . '_segment_' . asset_template_safe_sheet_name((string)($profile['segment_id'] ?? 'segment')) . '.xlsx';
...
function import_asset_common_admin_rows_from_template(int $profileId, array $file): array
{
    if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK || empty($file['tmp_name'])) {
        throw new RuntimeException('Please choose a valid Excel file.');
    }
    ...
    $existingRows = get_asset_common_admin_rows_detailed($profileId);
    $lockedRowCount = count($existingRows);
    $scopeOptions = array_flip(asset_common_scope_options());
    $fieldMetaById = [];
    $fieldMapByKey = [];
    foreach (get_asset_fields(true, (int)$profile['segment_id']) as $fieldRow) {
        $fieldMetaById[(int)$fieldRow['id']] = $fieldRow;
        $fieldMapByKey[(string)$fieldRow['field_key']] = $fieldRow;
    }
    ...
    if (!$rows || count($rows) < 2) {
        throw new RuntimeException('Template has no data rows.');
    }
    $expectedColumns = count($profileFields) + 3;
    ...
    if ($headerCount !== $expectedColumns) {
        throw new RuntimeException('Data sheet column count must match SL, Office Name, common fields, and Instruction.');
    }
    ...
    foreach ($rows as $index => $row) {
        ...
        $scopeLabel = trim((string)($row[1] ?? ''));
        $hasPayload = $scopeLabel !== '';
        foreach ($profileFields as $offset => $profileField) {
            if (trim((string)($row[$offset + 2] ?? '')) !== '') {
                $hasPayload = true;
                break;
            }
        }
        ...
        if (!isset($scopeOptions[$scopeLabel])) {
            throw new RuntimeException('Invalid scope label found in template row ' . ($index + 1) . '.');
        }
        ...
        $parsedRows[] = [
            ...
            'scope_office_id' => (int)$scope['office_id'],
            'sort_order' => max(1, $index * 10),
            'values' => $values,
        ];
    }
    ...
    db()->beginTransaction();
    try {
        $maxSortStmt = db()->prepare('SELECT COALESCE(MAX(sort_order), 0) FROM asset_common_admin_rows WHERE profile_id = ?');
        $maxSortStmt->execute([$profileId]);
        $nextSortOrder = (int)$maxSortStmt->fetchColumn();
        ...
        foreach ($parsedRows as $parsedRow) {
            ...
            db()->prepare('INSERT INTO asset_common_admin_rows (profile_id, stable_row_key, scope_office_type, scope_office_id, sort_order, active_status, created_at) VALUES (?, ?, ?, ?, ?, 1, NOW())')->execute([
                $profileId,
                asset_common_admin_row_key(),
                $parsedRow['scope_office_type'],
                $parsedRow['scope_office_id'],
                $nextSortOrder,
            ]);
            $rowId = (int)db()->lastInsertId();
            foreach ($parsedRow['values'] as $childFieldId => $valueText) {
                db()->prepare('INSERT INTO asset_common_admin_row_values (row_id, child_field_id, value_text, created_at) VALUES (?, ?, ?, NOW())')->execute([$rowId, $childFieldId, $valueText]);
            }
        }
        db()->commit();
    ...
    return ['imported' => count($parsedRows)];
}

function asset_common_existing_generated_assets(int $profileId, int $officeType, int $officeId): array
{
    $stmt = db()->prepare(
        'SELECT id, common_row_key, deleted_at
         FROM assets
         WHERE common_profile_id = ?
           AND office_type = ?
           AND office_id = ?
           AND is_user_added_row = 0'
    );
    $stmt->execute([$profileId, $officeType, $officeId]);
    $map = [];
    foreach ($stmt->fetchAll() as $row) {
        $map[(string)$row['common_row_key']] = $row;
    }
    return $map;
}

function asset_common_child_field_map(int $segmentId): array
{
    $fieldMap = [];
...
function asset_common_normalize_child_field_value(array $childField, mixed $rawValue, array $fieldMap): array
{
    $errors = [];
    return normalize_asset_field_value($childField, $rawValue, $fieldMap, $errors);
}

function asset_common_build_user_defined_rows(array $profile, array $bindings, array $childFieldMap, int $officeType, int $officeId): array
{
    $parentSegmentId = (int)($profile['parent_segment_id'] ?? 0);
    $parentCategoryId = (int)($profile['parent_category_id'] ?? 0);
    if ($parentSegmentId <= 0 || $parentCategoryId <= 0) {
        return [];
    }
    $sourceAssets = asset_common_fetch_source_assets($parentSegmentId, $parentCategoryId, $officeType, $officeId);
    if ($sourceAssets === []) {
        return [];
    }
    $fieldMapByKey = [];
    ...
    foreach ($sourceAssets as $sourceAsset) {
        ...
        foreach ($bindings as $binding) {
            $childFieldId = (int)($binding['child_field_id'] ?? 0);
            $parentFieldKey = trim((string)($binding['parent_field_key'] ?? ''));
            if ($childFieldId <= 0 || $parentFieldKey === '' || !isset($childFieldMap[$childFieldId])) {
                continue;
            }
            $childField = $childFieldMap[$childFieldId];
            $normalized = asset_common_normalize_child_field_value(
                $childField,
                $sourceAsset['values'][$parentFieldKey] ?? null,
                $fieldMapByKey
            );
        ...
        $desiredRows[] = [
            ...
            'source_asset_id' => (int)$sourceAsset['id'],
            'field_values' => $fieldValues,
            'context_asset' => $sourceAsset,
        ];
    }
    return $desiredRows;
}

function asset_common_build_superadmin_defined_rows(array $profile, array $bindings, array $childFieldMap, int $officeType, int $officeId): array
{
    $adminRows = asset_common_fetch_admin_rows((int)$profile['id'], $officeType, $officeId);
    if ($adminRows === []) {
        return [];
    }
    $fieldMapByKey = [];
    ...
    foreach ($adminRows as $adminRow) {
        ...
        foreach ($bindings as $binding) {
            ...
            $normalized = asset_common_normalize_child_field_value(
                $childField,
                $adminRow['values'][$childFieldId] ?? null,
                $fieldMapByKey
            );
            $fieldValues[(string)$childField['field_key']] = $normalized;
        }
...
function asset_common_admin_row_order_map_for_profile(int $profileId): array
{
    static $cache = [];
    if ($profileId <= 0) {
        return [];
    }
    if (array_key_exists($profileId, $cache)) {
        return $cache[$profileId];
    }
    $map = [];
    foreach (get_asset_common_admin_rows_detailed($profileId) as $row) {
        $stableRowKey = trim((string)($row['stable_row_key'] ?? ''));
        if ($stableRowKey === '') {
            continue;
        }
        $map[$stableRowKey] = (int)($row['sort_order'] ?? 0);
    }
    $cache[$profileId] = $map;
    return $map;
}

function asset_common_admin_row_scope_meta_for_profile(int $profileId): array
{
    static $cache = [];
    if ($profileId <= 0) {
        return [];
...
function asset_common_scope_matches_office(int $scopeOfficeType, int $scopeOfficeId, int $officeType, int $officeId): bool
{
    if ($scopeOfficeType <= 0 && $scopeOfficeId <= 0) {
        return true;
    }
    if ($scopeOfficeType !== $officeType) {
        return false;
    }
    if ($scopeOfficeId <= 0) {
        return true;
    }
    return $scopeOfficeId === $officeId;
}

function asset_common_generated_superadmin_row_is_currently_applicable(array $asset): bool
{
    $profileId = (int)($asset['common_profile_id'] ?? 0);
    if ($profileId <= 0) {
        return true;
    }
    if ((int)($asset['is_user_added_row'] ?? 1) !== 0) {
        return true;
    }
    if ((string)($asset['common_source_type'] ?? '') !== asset_common_source_type_superadmin_defined()) {
        return true;
    }
    $rowKey = trim((string)($asset['common_row_key'] ?? ''));
    if ($rowKey === '') {
        return false;
    }
    $scopeMetaMap = asset_common_admin_row_scope_meta_for_profile($profileId);
    $scopeMeta = $scopeMetaMap[$rowKey] ?? null;
    if (!$scopeMeta) {
        return false;
    }
    return asset_common_scope_matches_office(
        (int)($scopeMeta['scope_office_type'] ?? 0),
        (int)($scopeMeta['scope_office_id'] ?? 0),
        (int)($asset['office_type'] ?? 0),
        (int)($asset['office_id'] ?? 0)
    );
}

function asset_apply_superadmin_common_default_order(array $rows): array
{
    if ($rows === []) {
        return $rows;
    }
    usort($rows, static function (array $left, array $right): int {
        $leftProfileId = (int)($left['common_profile_id'] ?? 0);
        $rightProfileId = (int)($right['common_profile_id'] ?? 0);
        $leftGenerated = $leftProfileId > 0
            && (int)($left['is_user_added_row'] ?? 1) === 0
            && (string)($left['common_source_type'] ?? '') === asset_common_source_type_superadmin_defined();
        $rightGenerated = $rightProfileId > 0
            && (int)($right['is_user_added_row'] ?? 1) === 0
            && (string)($right['common_source_type'] ?? '') === asset_common_source_type_superadmin_defined();

        if ($leftGenerated || $rightGenerated) {
            if ($leftGenerated !== $rightGenerated) {
                return $leftGenerated ? -1 : 1;
            }
            $leftOrderMap = asset_common_admin_row_order_map_for_profile($leftProfileId);
            $rightOrderMap = asset_common_admin_row_order_map_for_profile($rightProfileId);
        ...
        $categoryNameCompare = strnatcasecmp((string)($left['category_name'] ?? ''), (string)($right['category_name'] ?? ''));
        if ($categoryNameCompare !== 0) {
            return $categoryNameCompare;
        }
        $manualCompare = ((int)($left['is_user_added_row'] ?? 1)) <=> ((int)($right['is_user_added_row'] ?? 1));
        if ($manualCompare !== 0) {
...
function asset_common_insert_generated_asset(array $profile, int $officeType, int $officeId, string $rowKey, string $sourceType, ?int $sourceAssetId, int $actorUserId): int
{
    $stmt = db()->prepare(
        'INSERT INTO assets (
            segment_id, asset_number, category_id, subcategory_id, common_profile_id, common_row_key,
            common_source_type, common_source_asset_id, is_user_added_row, office_type, office_id,
            active_status, created_by, created_at
        ) VALUES (?, ?, ?, NULL, ?, ?, ?, ?, 0, ?, ?, 1, ?, NOW())'
    );
    $stmt->execute([
        (int)$profile['segment_id'],
        'PENDING',
        (int)$profile['category_id'],
        (int)$profile['id'],
        $rowKey,
        $sourceType,
        $sourceAssetId,
...
function asset_common_upsert_generated_asset(array $profile, int $officeType, int $officeId, array $desiredRow, int $actorUserId): void
{
    $existingRows = asset_common_existing_generated_assets((int)$profile['id'], $officeType, $officeId);
    $existingRow = $existingRows[(string)$desiredRow['row_key']] ?? null;
    $beforeAsset = $existingRow ? get_asset((int)$existingRow['id'], true) : null;
    $beforeSignature = asset_common_activity_signature($beforeAsset);
    if ($existingRow) {
        $assetId = (int)$existingRow['id'];
        db()->prepare(
            'UPDATE assets
             SET segment_id = ?, category_id = ?, subcategory_id = NULL, common_profile_id = ?, common_row_key = ?,
                 common_source_type = ?, common_source_asset_id = ?, is_user_added_row = 0, office_type = ?, office_id = ?,
                 active_status = 1, deleted_at = NULL, updated_by = ?, updated_at = NOW()
             WHERE id = ?'
        )->execute([
            (int)$profile['segment_id'],
            (int)$profile['category_id'],
            (int)$profile['id'],
            (string)$desiredRow['row_key'],
            (string)$desiredRow['source_type'],
            $desiredRow['source_asset_id'] !== null ? (int)$desiredRow['source_asset_id'] : null,
            $officeType,
            $officeId,
            $actorUserId,
            $assetId,
        ...
        $assetId = asset_common_insert_generated_asset(
            $profile,
            $officeType,
            $officeId,
            (string)$desiredRow['row_key'],
    ...
    if (!$beforeAsset && $afterAsset) {
        add_asset_activity_log_with_segment(
            (int)($afterAsset['segment_id'] ?? asset_default_segment_id()),
            $assetId,
            $actorUserId,
            'common_row_generated',
            'Common row generated.',
            asset_common_activity_details($afterAsset)
        );
    } elseif ($beforeAsset && $afterAsset && $beforeSignature !== $afterSignature) {
        add_asset_activity_log_with_segment(
            (int)($afterAsset['segment_id'] ?? asset_default_segment_id()),
            $assetId,
            $actorUserId,
            'common_row_synced',
            'Common row synchronized.',
            asset_common_activity_details($afterAsset)
        );
    }
}

function asset_common_cleanup_generated_assets(array $profile, int $officeType, int $officeId, array $desiredRowKeys, int $actorUserId): void
{
    $existingRows = asset_common_existing_generated_assets((int)$profile['id'], $officeType, $officeId);
    $desiredLookup = array_fill_keys($desiredRowKeys, true);
    foreach ($existingRows as $rowKey => $existingRow) {
        if (isset($desiredLookup[$rowKey])) {
            continue;
        }
        $assetId = (int)($existingRow['id'] ?? 0);
        if ($assetId <= 0) {
            continue;
        }
        $beforeAsset = get_asset($assetId, true);
        ...
        add_asset_activity_log_with_segment(
            ...
            $actorUserId,
            'common_row_removed',
            'Common row removed after source change.',
            asset_common_activity_details($beforeAsset)
        );
    }
}

function asset_sync_common_rows_for_profile_office(int $profileId, int $officeType, int $officeId, ?int $actingUserId = null): void
{
    $profile = get_asset_common_profile($profileId);
    if (!$profile || (int)($profile['active_status'] ?? 0) !== 1 || $officeType <= 0 || $officeId <= 0) {
        return;
    }
    $bindings = get_asset_common_profile_fields($profileId);
    if ($bindings === []) {
        return;
    }
    $childFieldMap = asset_common_child_field_map((int)$profile['segment_id']);
    $desiredRows = (string)($profile['definition_mode'] ?? '') === asset_common_definition_mode_superadmin_defined()
        ? asset_common_build_superadmin_defined_rows($profile, $bindings, $childFieldMap, $officeType, $officeId)
        : asset_common_build_user_defined_rows($profile, $bindings, $childFieldMap, $officeType, $officeId);

    $actorUserId = asset_common_sync_actor_user_id($actingUserId, $desiredRows[0]['context_asset'] ?? null);
    db()->beginTransaction();
    try {
        $desiredRowKeys = [];
        foreach ($desiredRows as $desiredRow) {
            $desiredRowKeys[] = (string)$desiredRow['row_key'];
            asset_common_upsert_generated_asset($profile, $officeType, $officeId, $desiredRow, $actorUserId);
        }
        asset_common_cleanup_generated_assets($profile, $officeType, $officeId, $desiredRowKeys, $actorUserId);
        db()->commit();
    } catch (Throwable $e) {
        if (db()->inTransaction()) {
            db()->rollBack();
...
function asset_sync_common_rows_for_profile_across_offices(int $profileId, ?int $actingUserId = null): void
{
    $profile = get_asset_common_profile($profileId);
    if (!$profile) {
        return;
    }
    $scopes = asset_common_sync_target_offices_for_profile($profile);
    foreach ($scopes as $scope) {
        asset_sync_common_rows_for_profile_office(
            $profileId,
            (int)($scope['office_type'] ?? 0),
            (int)($scope['office_id'] ?? 0),
            $actingUserId
        );
    }
}

function asset_sync_common_rows_for_segment_office(int $segmentId, int $officeType, int $officeId, ?int $actingUserId = null): void
{
    if ($segmentId <= 0 || $officeType <= 0 || $officeId <= 0) {
        return;
    }
    foreach (get_asset_common_profiles_for_segment($segmentId) as $profile) {
        asset_sync_common_rows_for_profile_office((int)$profile['id'], $officeType, $officeId, $actingUserId);
    }
}

function asset_sync_common_rows_after_asset_change(?array $beforeAsset, ?array $afterAsset, ?int $actingUserId = null): void
{
    $segmentScopes = [];
    $profileScopes = [];
    $queueSegmentScope = static function (?array $asset) use (&$segmentScopes): void {
        if (!$asset) {
            return;
        }
        $segmentId = (int)($asset['segment_id'] ?? 0);
        $officeType = (int)($asset['office_type'] ?? 0);
        $officeId = (int)($asset['office_id'] ?? 0);
        if ($segmentId <= 0 || $officeType <= 0 || $officeId <= 0) {
            return;
        }
        $segmentScopes[$segmentId . ':' . $officeType . ':' . $officeId] = [$segmentId, $officeType, $officeId];
    };
    $queueDependentProfiles = static function (?array $asset) use (&$profileScopes): void {
        if (!$asset) {
            return;
        }
        $segmentId = (int)($asset['segment_id'] ?? 0);
        ...
        foreach (get_asset_common_profiles_by_parent($segmentId, $categoryId) as $profile) {
            $profileScopes[(int)$profile['id'] . ':' . $officeType . ':' . $officeId] = [(int)$profile['id'], $officeType, $officeId];
        }
    };
    $queueSegmentScope($beforeAsset);
    $queueSegmentScope($afterAsset);
    $queueDependentProfiles($beforeAsset);
    $queueDependentProfiles($afterAsset);

    foreach ($segmentScopes as [$segmentId, $officeType, $officeId]) {
        asset_sync_common_rows_for_segment_office($segmentId, $officeType, $officeId, $actingUserId);
    }
    foreach ($profileScopes as [$profileId, $officeType, $officeId]) {
        asset_sync_common_rows_for_profile_office($profileId, $officeType, $officeId, $actingUserId);
    }
}
...
function asset_tutorial_video_storage_dir(): string
{
    return dirname(__DIR__, 2) . '/storage/tutorial_videos';
}

function asset_tutorial_video_manual_path_note(): string
{
    return 'storage/tutorial_videos/';
}

function asset_tutorial_video_ui_max_bytes(): int
{
    return 500 * 1024 * 1024;
}

function asset_ensure_tutorial_video_storage_dir(): string
{
    $dir = asset_tutorial_video_storage_dir();
    if (!is_dir($dir) && !mkdir($dir, 0777, true) && !is_dir($dir)) {
        throw new RuntimeException('Unable to create tutorial video storage directory.');
    }
    return $dir;
}

function asset_normalize_tutorial_video_filename(?string $value): string
{
    $value = trim((string)$value);
    if ($value === '') {
        return '';
...
function asset_extract_uploaded_tutorial_video(array $file): ?array
{
    $tmpName = (string)($file['tmp_name'] ?? '');
    $name = (string)($file['name'] ?? '');
    $error = (int)($file['error'] ?? UPLOAD_ERR_NO_FILE);
    if ($error === UPLOAD_ERR_NO_FILE && $tmpName === '' && $name === '') {
        return null;
    }
    return [
        'name' => $name,
...
function asset_validate_tutorial_video_input(?array $upload, string $manualFilename, bool $removeExisting, string $label, array &$errors): void
{
    if ($removeExisting && ($upload || $manualFilename !== '')) {
        $errors[] = $label . ': remove existing cannot be combined with upload or server filename.';
        return;
    }
    if ($upload && $manualFilename !== '') {
        $errors[] = $label . ': choose either UI upload or server filename.';
        return;
    }
    if ($upload) {
        $error = (int)($upload['error'] ?? UPLOAD_ERR_NO_FILE);
        if ($error !== UPLOAD_ERR_OK) {
            $errors[] = $label . ': UI upload failed. For videos larger than 500 MB, upload the MP4 file to ' . asset_tutorial_video_manual_path_note() . ' and register the filename here.';
            return;
        }
        $extension = strtolower(pathinfo((string)($upload['name'] ?? ''), PATHINFO_EXTENSION));
        if ($extension !== 'mp4') {
            $errors[] = $label . ': only MP4 tutorial videos are allowed.';
        }
        if ((int)($upload['size'] ?? 0) > asset_tutorial_video_ui_max_bytes()) {
            $errors[] = $label . ': UI upload supports MP4 up to 500 MB. Larger files upload by FileZilla to ' . asset_tutorial_video_manual_path_note() . ' and register the filename here.';
        }
        return;
    }
    if ($manualFilename !== '') {
        $normalized = asset_normalize_tutorial_video_filename($manualFilename);
        if ($normalized === '') {
            $errors[] = $label . ': server filename is invalid. Use only the MP4 filename stored in ' . asset_tutorial_video_manual_path_note();
            return;
        }
        if (strtolower(pathinfo($normalized, PATHINFO_EXTENSION)) !== 'mp4') {
            $errors[] = $label . ': server filename must be an MP4 file.';
        }
    }
}

function asset_field_has_hosted_tutorial(array $field): bool
{
    return trim((string)($field['hosted_tutorial_video_path'] ?? '')) !== '';
}

function asset_field_hosted_tutorial_stream_url(array $field): ?string
{
    $id = (int)($field['id'] ?? 0);
    if ($id <= 0 || !asset_field_has_hosted_tutorial($field)) {
        return null;
    }
    return 'index.php?page=field_tutorial_video&id=' . $id;
}

function asset_cleanup_previous_uploaded_tutorial(array $field): void
{
    $stored = trim((string)($field['hosted_tutorial_video_path'] ?? ''));
    if ($stored === '') {
        return;
    }
    if (!preg_match('/^field-\d+-\d+\.mp4$/i', $stored)) {
        return;
    }
    $path = asset_tutorial_video_storage_dir() . DIRECTORY_SEPARATOR . $stored;
    if (is_file($path)) {
        @unlink($path);
    }
}

function asset_store_uploaded_tutorial_video(array $upload, int $fieldId, array &$createdFiles): array
{
    $dir = asset_ensure_tutorial_video_storage_dir();
    $storedName = 'field-' . $fieldId . '-' . time() . '.mp4';
    $target = $dir . DIRECTORY_SEPARATOR . $storedName;
    if (!move_uploaded_file((string)$upload['tmp_name'], $target)) {
        throw new RuntimeException('Unable to store hosted tutorial video.');
    }
    $createdFiles[] = $target;
    return [
        'path' => $storedName,
        'original_name' => trim((string)($upload['name'] ?? '')) ?: $storedName,
        'size' => (int)($upload['size'] ?? 0),
    ];
}

function asset_resolve_manual_tutorial_video(string $manualFilename): array
{
    $normalized = asset_normalize_tutorial_video_filename($manualFilename);
    if ($normalized === '') {
        throw new RuntimeException('Hosted tutorial server filename is invalid.');
    }
    $path = asset_tutorial_video_storage_dir() . DIRECTORY_SEPARATOR . $normalized;
    if (!is_file($path)) {
        throw new RuntimeException('Hosted tutorial video was not found in ' . asset_tutorial_video_manual_path_note());
    }
    return [
        'path' => $normalized,
...
function asset_assign_field_tutorial_video(int $fieldId, ?array $upload, string $manualFilename, bool $removeExisting, array &$createdFiles = []): void
{
    $field = get_asset_field_any_segment($fieldId);
    if (!$field) {
        throw new RuntimeException('Field not found for hosted tutorial video.');
    }

    if ($removeExisting) {
        asset_cleanup_previous_uploaded_tutorial($field);
        $stmt = db()->prepare('UPDATE asset_fields SET hosted_tutorial_video_path = NULL, hosted_tutorial_video_original_name = NULL, hosted_tutorial_video_size = NULL, updated_at = NOW() WHERE id = ?');
    ...
    if ($upload) {
        asset_cleanup_previous_uploaded_tutorial($field);
        $stored = asset_store_uploaded_tutorial_video($upload, $fieldId, $createdFiles);
        $stmt = db()->prepare('UPDATE asset_fields SET hosted_tutorial_video_path = ?, hosted_tutorial_video_original_name = ?, hosted_tutorial_video_size = ?, updated_at = NOW() WHERE id = ?');
        $stmt->execute([$stored['path'], $stored['original_name'], $stored['size'], $fieldId]);
        return;
    }

    $manualFilename = trim($manualFilename);
    if ($manualFilename !== '') {
        asset_cleanup_previous_uploaded_tutorial($field);
        $stored = asset_resolve_manual_tutorial_video($manualFilename);
        $stmt = db()->prepare('UPDATE asset_fields SET hosted_tutorial_video_path = ?, hosted_tutorial_video_original_name = ?, hosted_tutorial_video_size = ?, updated_at = NOW() WHERE id = ?');
        $stmt->execute([$stored['path'], $stored['original_name'], $stored['size'], $fieldId]);
    }
}

function get_asset_field_any_segment(int $fieldId): ?array
{
...
function stream_field_tutorial_video(int $fieldId): void
{
    $field = get_asset_field_any_segment($fieldId);
    if (!$field || !asset_field_has_hosted_tutorial($field)) {
        http_response_code(404);
        exit('Tutorial video not found.');
    }
    $storedName = asset_normalize_tutorial_video_filename((string)$field['hosted_tutorial_video_path']);
    $path = asset_tutorial_video_storage_dir() . DIRECTORY_SEPARATOR . $storedName;
    ...
    if ($rangeHeader !== '' && preg_match('/bytes=(\d*)-(\d*)/i', $rangeHeader, $matches)) {
        ...
        $rangeEnd = $matches[2] !== '' ? (int)$matches[2] : ($size - 1);
        if ($rangeStart < 0) {
            $rangeStart = 0;
        }
        if ($rangeEnd >= $size) {
    ...
    header('Content-Disposition: inline; filename="' . rawurlencode((string)($field['hosted_tutorial_video_original_name'] ?? $storedName)) . '"');
    if ($status === 206) {
    ...
    if ($handle === false) {
        ...
        exit('Unable to open tutorial video.');
    }
    try {
        fseek($handle, $start);
...
function asset_field_has_help(array $field): bool
{
    return asset_normalize_help_text((string)($field['field_information'] ?? '')) !== ''
        ...
        || asset_field_has_hosted_tutorial($field);
}

function asset_global_tutorial_has_hosted_video(?array $info): bool
{
    return trim((string)($info['hosted_tutorial_video_path'] ?? '')) !== '';
}

function asset_global_tutorial_stream_url(?array $info): ?string
{
    if (!asset_global_tutorial_has_hosted_video($info)) {
        return null;
    }
    return 'index.php?page=global_tutorial_video';
}

function asset_cleanup_previous_global_tutorial(?array $info): void
{
    $stored = asset_normalize_tutorial_video_filename((string)($info['hosted_tutorial_video_path'] ?? ''));
    if ($stored === '' || !preg_match('/^global-tutorial-\d+\.mp4$/i', $stored)) {
        return;
    }
    $path = asset_tutorial_video_storage_dir() . DIRECTORY_SEPARATOR . $stored;
    if (is_file($path)) {
...
function asset_store_global_tutorial_video(array $upload, array &$createdFiles): array
{
    $dir = asset_ensure_tutorial_video_storage_dir();
    $storedName = 'global-tutorial-' . time() . '.mp4';
    $target = $dir . DIRECTORY_SEPARATOR . $storedName;
    if (!move_uploaded_file((string)$upload['tmp_name'], $target)) {
        throw new RuntimeException('Unable to store global tutorial video.');
    }
    $createdFiles[] = $target;
    return [
        'path' => $storedName,
...
function asset_assign_global_tutorial_video(?array $upload, string $manualFilename, bool $removeExisting, array &$createdFiles = []): array
{
    $info = get_info_row() ?? [];
    $currentPath = trim((string)($info['hosted_tutorial_video_path'] ?? ''));
    $currentOriginalName = trim((string)($info['hosted_tutorial_video_original_name'] ?? ''));
    $currentSize = (int)($info['hosted_tutorial_video_size'] ?? 0);

    if ($removeExisting) {
        asset_cleanup_previous_global_tutorial($info);
        return [
            'hosted_tutorial_video_path' => null,
            'hosted_tutorial_video_original_name' => null,
            'hosted_tutorial_video_size' => null,
        ];
    }

    if ($upload) {
        asset_cleanup_previous_global_tutorial($info);
        $stored = asset_store_global_tutorial_video($upload, $createdFiles);
        return [
            'hosted_tutorial_video_path' => $stored['path'],
            'hosted_tutorial_video_original_name' => $stored['original_name'],
            'hosted_tutorial_video_size' => $stored['size'],
        ];
    }
    ...
    return [
        'hosted_tutorial_video_path' => $currentPath !== '' ? $currentPath : null,
        'hosted_tutorial_video_original_name' => $currentOriginalName !== '' ? $currentOriginalName : null,
        'hosted_tutorial_video_size' => $currentSize > 0 ? $currentSize : null,
    ];
}

function stream_global_tutorial_video(): void
{
    $info = get_info_row();
    if (!$info || !asset_global_tutorial_has_hosted_video($info)) {
        http_response_code(404);
        exit('Tutorial video not found.');
    }
    $storedName = asset_normalize_tutorial_video_filename((string)$info['hosted_tutorial_video_path']);
    $path = asset_tutorial_video_storage_dir() . DIRECTORY_SEPARATOR . $storedName;
    if ($storedName === '' || !is_file($path)) {
        http_response_code(404);
        exit('Tutorial video not found.');
    ...
    header('Content-Length: ' . max(0, ($end - $start) + 1));
    header('Accept-Ranges: bytes');
    header('Content-Disposition: inline; filename="' . rawurlencode((string)($info['hosted_tutorial_video_original_name'] ?? $storedName)) . '"');
    if ($status === 206) {
        header('Content-Range: bytes ' . $start . '-' . $end . '/' . $size);
    }
...
function asset_normalize_option_compare_value(string $value): string
{
...
function asset_option_values_match(mixed $left, mixed $right): bool
{
    return asset_normalize_option_compare_value((string)$left) === asset_normalize_option_compare_value((string)$right);
}

function asset_resolve_field_option_value(array $field, mixed $value): ?string
{
    $value = trim((string)$value);
    if ($value === '') {
        return null;
    ...
    foreach (get_asset_field_options((int)$field['id']) as $option) {
        ...
        if (asset_option_values_match($optionValue, $value) || asset_option_values_match($optionLabel, $value)) {
            return $optionValue;
        }
    }
    return null;
...
function asset_resolve_conditional_primary_value(array $field, mixed $value): ?string
{
    $value = trim((string)$value);
    if ($value === '') {
        return null;
    }
    foreach (asset_decode_conditional_map($field) as $primary => $options) {
        if (asset_option_values_match($primary, $value)) {
            return $primary;
        }
    }
    return asset_resolve_field_option_value($field, $value);
}
...
function asset_conditional_child_options(array $parentField, string $parentValue): array
{
    ...
    $resolvedPrimary = asset_resolve_conditional_primary_value($parentField, $parentValue);
    if ($resolvedPrimary !== null && isset($map[$resolvedPrimary])) {
        return $map[$resolvedPrimary];
    }
    return [];
}

function asset_resolve_conditional_child_value(array $parentField, mixed $parentValue, mixed $childValue): ?string
{
    $childValue = trim((string)$childValue);
    if ($childValue === '') {
        return null;
    }
    foreach (asset_conditional_child_options($parentField, (string)$parentValue) as $option) {
        if (asset_option_values_match($option, $childValue)) {
            return $option;
        }
    }
    return null;
...
function asset_field_fill_color_palette(): array
{
    return [
        '' => 'No color',
        '#F5F7FA' => 'Light gray',
...
function asset_normalize_field_fill_color(?string $value): string
{
    $value = strtoupper(trim((string)$value));
    $palette = asset_field_fill_color_palette();
    if ($value === '' || !isset($palette[$value])) {
        return '';
    }
    return $value;
}
...
function asset_core_columns(): array
{
    return [
        ...
        ['key' => 'asset_number', 'label' => 'Asset Number / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â®ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂªÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡'],
        ['key' => 'subcategory_name', 'label' => 'Sub-category / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âª-ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¶ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿'],
    ];
}
...
function asset_office_type_provides_data(int $officeType): bool
{
    $segments = get_asset_segments(true);
    if (!$segments) {
        return true;
    ...
    foreach ($segments as $segment) {
        $settings = asset_scope_visibility_for_office_type($officeType, (int)($segment['id'] ?? 0));
        if (!empty($settings['show_my_office'])) {
            return true;
        }
    }
    return false;
...
function asset_segment_office_type_provides_data(int $officeType, int $segmentId): bool
{
    $settings = asset_scope_visibility_for_office_type($officeType, $segmentId);
    return !empty($settings['show_my_office']);
}
...
function asset_filter_index_blank_token(): string
{
    return '__blank__';
}

function asset_filter_index_no_file_token(): string
{
    return '__no_file__';
}

function asset_filter_index_supported_data_types(): array
{
    return ['text', 'number', 'file', 'bimh'];
}

function asset_filter_index_field_is_trackable(array $field): bool
{
    $dataType = strtolower(trim((string)($field['data_type'] ?? '')));
    if (!in_array($dataType, asset_filter_index_supported_data_types(), true)) {
        return false;
    }
    ...
    return $scope !== asset_filter_scope_none();
}

function asset_filter_index_sort_value(string $value): string
{
...
function asset_filter_index_build_items_for_field_asset(array $asset, array $field): array
{
    if (!asset_filter_index_field_is_trackable($field)) {
        return [];
    }
    $segmentId = asset_normalize_segment_id((int)($asset['segment_id'] ?? 0));
    $fieldId = (int)($field['id'] ?? 0);
    $fieldKey = (string)($field['field_key'] ?? '');
    $dataType = strtolower(trim((string)($field['data_type'] ?? '')));
    if ($segmentId <= 0 || $fieldId <= 0 || $fieldKey === '') {
        return [];
    }

    $items = [];
    $register = static function (string $itemKind, string $itemValue, string $itemLabel) use (&$items, $segmentId, $fieldId, $fieldKey, $dataType): void {
        $key = $itemKind . '|' . $itemValue;
        $items[$key] = [
            'segment_id' => $segmentId,
            'field_id' => $fieldId,
            'field_key' => $fieldKey,
            ...
            'item_sort_value' => asset_filter_index_sort_value($itemLabel),
        ];
    };
    ...
    if ($dataType === 'file') {
        ...
        if ($extensions === []) {
            $register('no_file', asset_filter_index_no_file_token(), 'No file');
            return array_values($items);
        }
        foreach ($extensions as $extension) {
            $register('extension', strtolower($extension), strtolower($extension));
        }
        return array_values($items);
    }

    $value = trim((string)($asset['values'][$fieldKey] ?? ''));
    if ($value === '') {
        $register('blank', asset_filter_index_blank_token(), 'Blank');
        return array_values($items);
    }
...
function asset_filter_index_fetch_items_for_field(int $fieldId, ?int $segmentId = null): array
{
    $normalizedSegmentId = asset_normalize_segment_id($segmentId);
    $stmt = db()->prepare(
        'SELECT * FROM asset_filter_items
         WHERE segment_id = ? AND field_id = ? AND active_status = 1 AND usage_count > 0
         ORDER BY item_kind ASC, item_sort_value ASC, item_label ASC'
    );
    $stmt->execute([$normalizedSegmentId, $fieldId]);
    return $stmt->fetchAll();
}

function asset_filter_index_catalog_for_fields(int $segmentId, array $fields): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    if ($segmentId <= 0) {
        return [];
    ...
    foreach ($fields as $field) {
        if (
            (int)($field['active_status'] ?? 0) === 1
            && !asset_is_conditional_secondary($field)
            && asset_filter_index_field_is_trackable($field)
        ) {
            $trackableFields[(int)$field['id']] = $field;
        }
    }
    if (!$trackableFields) {
        return [];
    }
    $placeholders = implode(',', array_fill(0, count($trackableFields), '?'));
    $params = [$segmentId];
    foreach (array_keys($trackableFields) as $fieldId) {
        $params[] = $fieldId;
    }
    $stmt = db()->prepare(
        "SELECT field_id, item_kind, item_value, item_label
         FROM asset_filter_items
         WHERE segment_id = ?
           AND active_status = 1
           AND usage_count > 0
           AND field_id IN ($placeholders)
         ORDER BY field_id ASC, item_sort_value ASC, item_label ASC"
    );
    $stmt->execute($params);
    $catalog = [];
    foreach ($stmt->fetchAll() as $row) {
        $fieldId = (int)($row['field_id'] ?? 0);
        $field = $trackableFields[$fieldId] ?? null;
        if (!$field) {
            continue;
        }
        $fieldKey = (string)($field['field_key'] ?? '');
        ...
        $itemKind = (string)($row['item_kind'] ?? 'value');
        $itemValue = (string)($row['item_value'] ?? '');
        $itemLabel = (string)($row['item_label'] ?? $itemValue);
        if ($itemKind === 'blank' || $itemValue === asset_filter_index_blank_token()) {
            $catalog[$fieldKey]['has_blank'] = true;
            continue;
        }
        if ($itemKind === 'no_file' || $itemValue === asset_filter_index_no_file_token()) {
            $catalog[$fieldKey]['options']['__no_file__'] = 'No file';
            continue;
        }
        $catalog[$fieldKey]['options'][$itemValue] = $itemLabel;
    }
    foreach ($catalog as &$fieldCatalog) {
        if (!empty($fieldCatalog['options'])) {
            natcasesort($fieldCatalog['options']);
        }
    }
    unset($fieldCatalog);
    return $catalog;
...
function asset_filter_index_clear_for_field(int $fieldId, ?int $segmentId = null): void
{
    $normalizedSegmentId = asset_normalize_segment_id($segmentId);
    db()->prepare('DELETE FROM asset_filter_items WHERE segment_id = ? AND field_id = ?')->execute([$normalizedSegmentId, $fieldId]);
}

function asset_filter_index_clear_for_fields(int $segmentId, array $fieldIds): void
{
    $fieldIds = array_values(array_filter(array_map('intval', $fieldIds), static fn(int $value): bool => $value > 0));
    if ($fieldIds === []) {
        return;
    }
    $placeholders = implode(',', array_fill(0, count($fieldIds), '?'));
    $params = array_merge([$segmentId], $fieldIds);
    $stmt = db()->prepare("DELETE FROM asset_filter_items WHERE segment_id = ? AND field_id IN ({$placeholders})");
    $stmt->execute($params);
}

function asset_filter_index_store_aggregates(array $aggregates): int
{
    if ($aggregates === []) {
        return 0;
    }
    $stmt = db()->prepare(
        'INSERT INTO asset_filter_items
            (segment_id, field_id, field_key, data_type, item_kind, item_value, item_label, item_sort_value, usage_count, active_status, created_at, updated_at)
         VALUES
            (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())
         ON DUPLICATE KEY UPDATE
            ...
            item_sort_value = VALUES(item_sort_value),
...
function asset_filter_index_rebuild_fields(int $segmentId, array $fieldIds = []): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    $fieldMap = asset_field_map_for_segment(true, $segmentId);
    $targetFields = [];
    $requestedFieldIds = array_values(array_filter(array_map('intval', $fieldIds), static fn(int $value): bool => $value > 0));
    foreach ($fieldMap as $field) {
        $fieldId = (int)($field['id'] ?? 0);
        if ($requestedFieldIds !== [] && !in_array($fieldId, $requestedFieldIds, true)) {
            continue;
        }
        if (asset_filter_index_field_is_trackable($field)) {
            $targetFields[$fieldId] = $field;
        } elseif ($requestedFieldIds !== [] && in_array($fieldId, $requestedFieldIds, true)) {
            $targetFields[$fieldId] = $field;
        }
    }

    $clearIds = $requestedFieldIds !== [] ? $requestedFieldIds : array_map(static fn(array $field): int => (int)$field['id'], $targetFields);
    if ($clearIds !== []) {
        asset_filter_index_clear_for_fields($segmentId, $clearIds);
    }

    $trackedFields = array_filter($targetFields, static fn(array $field): bool => asset_filter_index_field_is_trackable($field));
    if ($trackedFields === []) {
        return ['fields_rebuilt' => 0, 'items_stored' => 0];
    }
    ...
    foreach ($assets as $asset) {
        foreach ($trackedFields as $field) {
            foreach (asset_filter_index_build_items_for_field_asset($asset, $field) as $item) {
                $key = implode('|', [
                ...
                if (!isset($aggregates[$key])) {
                    $aggregates[$key] = $item + ['usage_count' => 0];
                }
                $aggregates[$key]['usage_count']++;
            }
        }
    }

    $stored = asset_filter_index_store_aggregates($aggregates);
    return ['fields_rebuilt' => count($trackedFields), 'items_stored' => $stored];
}

function asset_filter_index_rebuild_field(int $fieldId, ?int $segmentId = null): array
{
    $field = get_asset_field($fieldId, $segmentId);
    if (!$field) {
        if ($segmentId !== null) {
            asset_filter_index_clear_for_field($fieldId, $segmentId);
        }
        return ['fields_rebuilt' => 0, 'items_stored' => 0];
    }
    return asset_filter_index_rebuild_fields(asset_normalize_segment_id((int)($field['segment_id'] ?? $segmentId ?? 0)), [$fieldId]);
}

function asset_filter_index_rebuild_all(): array
{
    $fieldsRebuilt = 0;
    $itemsStored = 0;
    foreach (get_asset_segments(true) as $segment) {
        $summary = asset_filter_index_rebuild_fields((int)($segment['id'] ?? 0));
        $fieldsRebuilt += (int)($summary['fields_rebuilt'] ?? 0);
        $itemsStored += (int)($summary['items_stored'] ?? 0);
    }
    return ['fields_rebuilt' => $fieldsRebuilt, 'items_stored' => $itemsStored];
}

function asset_filter_index_refresh_field(int $fieldId, ?int $segmentId = null): array
{
    $field = get_asset_field($fieldId, $segmentId);
    if (!$field) {
        if ($segmentId !== null) {
    ...
    asset_filter_index_clear_for_field($fieldId, (int)($field['segment_id'] ?? $segmentId ?? 0));
    if (!asset_filter_index_field_is_trackable($field)) {
        return ['fields_rebuilt' => 0, 'items_stored' => 0];
    }
    return asset_filter_index_rebuild_field($fieldId, (int)($field['segment_id'] ?? $segmentId ?? 0));
}

function asset_filter_index_apply_item_delta(array $item, int $delta): void
{
    if ($delta === 0) {
        return;
    }
    $stmt = db()->prepare(
        'SELECT id, usage_count
         FROM asset_filter_items
         WHERE segment_id = ? AND field_id = ? AND item_kind = ? AND item_value = ?
         LIMIT 1'
    );
    $stmt->execute([
        (int)$item['segment_id'],
    ...
    $usageCount = max(0, (int)($row['usage_count'] ?? 0) + $delta);
    db()->prepare(
        'UPDATE asset_filter_items
         SET field_key = ?, data_type = ?, item_label = ?, item_sort_value = ?, usage_count = ?, active_status = ?, updated_at = NOW()
         WHERE id = ?'
    )->execute([
        (string)$item['field_key'],
        (string)$item['data_type'],
...
function asset_filter_index_snapshot_from_asset(array $asset): array
{
    if ((int)($asset['active_status'] ?? 0) !== 1 || !empty($asset['deleted_at'])) {
        return [];
    }
    $segmentId = asset_normalize_segment_id((int)($asset['segment_id'] ?? 0));
    $fieldMap = asset_field_map_for_segment(true, $segmentId);
...
function asset_filter_index_sync_asset_change(?array $beforeAsset, ?array $afterAsset): void
{
    if ($beforeAsset) {
        foreach (asset_filter_index_snapshot_from_asset($beforeAsset) as $item) {
            asset_filter_index_apply_item_delta($item, -1);
        }
    }
    if ($afterAsset) {
        foreach (asset_filter_index_snapshot_from_asset($afterAsset) as $item) {
            asset_filter_index_apply_item_delta($item, 1);
        }
    }
}
...
function add_asset_activity_log(int $assetId, int $userId, string $actionType, string $summary, array $details = []): void
{
    ...
    add_asset_activity_log_with_segment($segmentId, $assetId, $userId, $actionType, $summary, $details);
}

function add_asset_activity_log_with_segment(int $segmentId, int $assetId, int $userId, string $actionType, string $summary, array $details = []): void
{
    $stmt = db()->prepare(
...
function asset_board_perf_enabled(): bool
{
    ...
    $enabled = is_superadmin() && request_str('board_perf', '') === '1';
    return $enabled;
}

function asset_board_perf_mark(string $label, array $meta = []): void
{
    if (!asset_board_perf_enabled()) {
        return;
    }
    $now = microtime(true);
    $GLOBALS['asset_board_perf'] ??= [
        'start' => $now,
        'marks' => [],
    ];
    $start = (float)($GLOBALS['asset_board_perf']['start'] ?? $now);
    $GLOBALS['asset_board_perf']['marks'][] = [
        'label' => $label,
...
function asset_board_perf_render(): string
{
    if (!asset_board_perf_enabled()) {
        return '';
    }
    $marks = (array)($GLOBALS['asset_board_perf']['marks'] ?? []);
    if ($marks === []) {
        return '';
    }
    $parts = [];
    foreach ($marks as $mark) {
        $line = (string)($mark['label'] ?? 'step') . ': ' . (string)($mark['elapsed_ms'] ?? '0') . 'ms';
        $meta = (array)($mark['meta'] ?? []);
        if ($meta !== []) {
            $line .= ' ' . json_encode($meta, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
        $parts[] = $line;
    }
    return "<!-- board_perf\n" . implode("\n", $parts) . "\n-->";
...
{
    $segmentId = asset_normalize_segment_id($segmentId);
    if (asset_segment_has_common_row_configuration($segmentId)) {
        throw new RuntimeException('This segment already uses common fields. Remove the common-field setup before adding another category.');
    }
...
{
    asset_assert_segment_match(get_asset_category($id, $segmentId), $segmentId, 'Category');
    if (asset_segment_has_common_row_configuration(asset_normalize_segment_id($segmentId))) {
        throw new RuntimeException('Delete the common-field setup first before removing this category.');
    }
...
function get_asset_subcategories(?int $categoryId = null, bool $includeInactive = false, ?int $segmentId = null): array
{
    ...
    $normalizedSegmentId = asset_normalize_segment_id($segmentId);
    $normalizedCategoryId = ($categoryId !== null && $categoryId > 0) ? (int)$categoryId : 0;
    $cacheKey = ($includeInactive ? '1' : '0') . ':' . $normalizedSegmentId . ':' . $normalizedCategoryId;
    if (array_key_exists($cacheKey, $cache)) {
        return $cache[$cacheKey];
    }
...
function create_asset_field(array $payload): void
{
    ...
    try {
        ...
        $bindingChange = ['old_profile_id' => 0, 'new_profile_id' => 0];
        if ($fieldId > 0) {
            $payload['acting_user_id'] = (int)(current_user()['id'] ?? 0);
            $bindingChange = asset_upsert_common_profile_binding($fieldId, $payload);
        }
        db()->commit();
    } catch (Throwable $e) {
        if (db()->inTransaction()) {
    ...
    if ($fieldId > 0) {
        asset_filter_index_refresh_field($fieldId, $segmentId);
        asset_sync_common_profiles_after_binding_change($bindingChange, (int)(current_user()['id'] ?? 0));
    }
    if ($childId > 0) {
...
function update_asset_field(int $id, array $payload): void
{
    ...
    try {
        if (($payload['data_type'] ?? '') === 'conditional') {
            ...
            $childStmt = db()->prepare('UPDATE asset_fields SET label = ?, data_type = ?, field_information = ?, video_tutorial_url = ?, fill_color = ?, number_format_rule = NULL, text_max_length = NULL, mandatory_scope = ?, is_required = ?, is_displayed = ?, is_import_enabled = ?, is_unique = 0, is_filter_enabled = ?, filter_scope = ?, is_common_download_field = ?, is_download_level1 = 0, is_download_filter = ?, is_download_sort = ?, is_download_zip_file_selectable = 0, is_download_token = ?, sort_order = ?, updated_at = NOW() WHERE id = ?');
            $childStmt->execute([
                $payload['secondary_label'],
                'dropdown',
                $payload['secondary_field_information'] ?: null,
        ...
        $payload['acting_user_id'] = (int)(current_user()['id'] ?? 0);
        $bindingChange = asset_upsert_common_profile_binding($id, $payload);
        db()->commit();
    } catch (Throwable $e) {
        if (db()->inTransaction()) {
            db()->rollBack();
    ...
    asset_filter_index_refresh_field($id, $segmentId);
    asset_sync_common_profiles_after_binding_change($bindingChange, (int)(current_user()['id'] ?? 0));
    if ($childField) {
...
function set_asset_field_status(int $id, int $status, ?int $segmentId = null): void
{
    ...
    asset_filter_index_refresh_field($id, $normalizedSegmentId);
    if ($child) {
        asset_filter_index_refresh_field((int)$child['id'], $normalizedSegmentId);
    }
}
...
function delete_asset_field(int $id, ?int $segmentId = null): bool
{
    ...
    foreach ($linkedFieldIds as $fieldId) {
        $binding = get_asset_common_profile_field_binding_by_child_field($fieldId);
        if ($binding) {
            db()->prepare('DELETE FROM asset_common_profile_fields WHERE child_field_id = ?')->execute([$fieldId]);
            asset_delete_common_profile_if_empty((int)$binding['profile_id'], (int)(current_user()['id'] ?? 0));
        }
        db()->prepare('UPDATE asset_fields SET is_common_row_field = 0, updated_at = NOW() WHERE id = ?')->execute([$fieldId]);
    }
    asset_filter_index_clear_for_fields(asset_normalize_segment_id((int)($field['segment_id'] ?? $segmentId ?? 0)), $linkedFieldIds);
    foreach ($linkedFieldIds as $fieldId) {
...
function validate_asset_field_definition(array $input, array $fileInput = [], ?int $fieldId = null): array
{
    ...
    $fillColor = asset_normalize_field_fill_color((string)($input['fill_color'] ?? ''));
    $tutorialVideoUpload = asset_extract_uploaded_tutorial_video((array)($fileInput['tutorial_video_file'] ?? []));
    $hostedTutorialVideoFilename = trim((string)($input['hosted_tutorial_video_filename'] ?? ''));
    $removeHostedTutorialVideo = !empty($input['remove_hosted_tutorial_video']);
    $errors = [];
    if ($label === '') {
    ...
    asset_validate_tutorial_video_input($tutorialVideoUpload, $hostedTutorialVideoFilename, $removeHostedTutorialVideo, 'Hosted tutorial video', $errors);
    asset_validate_tutorial_video_input($tutorialVideoUpload, $hostedTutorialVideoFilename, $removeHostedTutorialVideo, 'Hosted tutorial video', $errors);
    if ($existingField && (
    ...
    $secondaryFillColor = asset_normalize_field_fill_color((string)($input['secondary_fill_color'] ?? ''));
    $secondaryTutorialVideoUpload = asset_extract_uploaded_tutorial_video((array)($fileInput['secondary_tutorial_video_file'] ?? []));
    $secondaryHostedTutorialVideoFilename = trim((string)($input['secondary_hosted_tutorial_video_filename'] ?? ''));
    $removeSecondaryHostedTutorialVideo = !empty($input['remove_secondary_hosted_tutorial_video']);
    $conditionalMap = [];
    $secondaryOptions = [];
    $secondaryFieldKey = '';
    if ($dataType === 'conditional') {
        ...
        asset_validate_tutorial_video_input($secondaryTutorialVideoUpload, $secondaryHostedTutorialVideoFilename, $removeSecondaryHostedTutorialVideo, 'Secondary hosted tutorial video', $errors);
        asset_validate_tutorial_video_input($secondaryTutorialVideoUpload, $secondaryHostedTutorialVideoFilename, $removeSecondaryHostedTutorialVideo, 'Secondary hosted tutorial video', $errors);
    ...
    $commonRowConfigPresent = asset_common_row_config_input_present($input);
    $isCommonRowField = $commonRowConfigPresent && !empty($input['is_common_row_field']) ? 1 : 0;
    $commonProfileCategoryId = 0;
    $commonDefinitionMode = asset_common_definition_mode_user_defined();
    $commonRowPolicy = asset_common_row_policy_fixed();
    $commonParentSegmentId = 0;
    $commonParentCategoryId = 0;
    $commonParentFieldId = 0;
    ...
    if ($commonRowConfigPresent && $isCommonRowField === 1) {
        ...
        $segmentCategoryCount = count($segmentCategories);
        if ($segmentCategoryCount > 1) {
            $errors[] = 'Common fields are allowed only in segments with zero or one category.';
        }
        $commonProfileCategoryId = $segmentCategoryCount === 1 ? (int)($segmentCategories[0]['id'] ?? 0) : 0;
        $commonCategory = $segmentCategoryCount === 1 ? ($segmentCategories[0] ?? null) : ['id' => 0, 'segment_id' => $segmentId, 'name' => 'Default'];

        $existingProfiles = get_asset_common_profiles_for_segment($segmentId, true);
        $existingProfile = $existingProfiles[0] ?? null;
        $commonDefinitionModeInput = trim((string)($input['common_definition_mode'] ?? ''));
        $commonRowPolicyInput = trim((string)($input['common_row_policy'] ?? ''));

        $commonDefinitionMode = $existingProfile
            ? (string)$existingProfile['definition_mode']
            : asset_normalize_common_definition_mode($commonDefinitionModeInput);
        $commonRowPolicy = $existingProfile
            ? (string)$existingProfile['row_policy']
            : asset_normalize_common_row_policy($commonRowPolicyInput);
        $commonParentSegmentId = $existingProfile
            ? asset_normalize_segment_id((int)($existingProfile['parent_segment_id'] ?? 0))
            : asset_normalize_segment_id((int)($input['common_parent_segment_id'] ?? 0));
        $commonParentCategoryId = $existingProfile
            ? (int)($existingProfile['parent_category_id'] ?? 0)
            : (int)($input['common_parent_category_id'] ?? 0);

        if ($existingProfile) {
            if ($commonDefinitionModeInput !== '' && $commonDefinitionModeInput !== $commonDefinitionMode) {
                $errors[] = 'This category already has a common row definition mode. Add matching fields only.';
            }
            if ($commonRowPolicyInput !== '' && $commonRowPolicyInput !== $commonRowPolicy) {
                $errors[] = 'This category already has a common row policy. Add matching fields only.';
            }
            if (
                $commonDefinitionMode === asset_common_definition_mode_user_defined()
                && (
                    ((int)($input['common_parent_segment_id'] ?? 0) > 0 && asset_normalize_segment_id((int)$input['common_parent_segment_id']) !== $commonParentSegmentId)
                    || ((int)($input['common_parent_category_id'] ?? 0) > 0 && (int)$input['common_parent_category_id'] !== $commonParentCategoryId)
                )
            ) {
                $errors[] = 'This category already points to another parent segment/category. All common row fields in the category must share the same parent table.';
            }
        } else {
            if ($commonDefinitionModeInput === '') {
                $errors[] = 'Common row definition mode is required.';
            }
            if ($commonDefinitionMode !== asset_common_definition_mode_superadmin_defined() && $commonRowPolicyInput === '') {
                $errors[] = 'Common row policy is required.';
            }
        }

        if ($commonDefinitionMode === asset_common_definition_mode_user_defined()) {
            $commonParentFieldId = (int)($input['common_parent_field_id'] ?? 0);
            $parentSegmentCategories = $commonParentSegmentId > 0 ? get_asset_categories(true, $commonParentSegmentId) : [];
            $parentSegmentCategoryCount = count($parentSegmentCategories);
            if ($parentSegmentCategoryCount > 1) {
                $errors[] = 'User-defined common fields can point only to a parent segment with zero or one category.';
            }
            $commonParentCategoryId = $parentSegmentCategoryCount === 1 ? (int)($parentSegmentCategories[0]['id'] ?? 0) : 0;
            $parentCategory = $commonParentCategoryId > 0 ? get_asset_category($commonParentCategoryId, $commonParentSegmentId) : ['id' => 0];
            $parentField = $commonParentFieldId > 0 ? get_asset_field($commonParentFieldId, $commonParentSegmentId) : null;
            if ($commonParentSegmentId <= 0 || ($parentSegmentCategoryCount === 1 && !$parentCategory)) {
                $errors[] = 'Parent segment is required for user-defined common rows.';
            }
            if (!$parentField) {
    ...
    return [
        ...
        'payload' => [
            ...
            'tutorial_video_upload' => $tutorialVideoUpload,
            ...
            'common_row_config_present' => $commonRowConfigPresent ? 1 : 0,
            'is_common_row_field' => $isCommonRowField,
            'common_profile_category_id' => $commonProfileCategoryId,
            'common_definition_mode' => $commonDefinitionMode,
            'common_row_policy' => $commonRowPolicy,
            'common_parent_segment_id' => $commonParentSegmentId,
            'common_parent_category_id' => $commonParentCategoryId,
            'common_parent_field_id' => $commonParentFieldId,
            'sort_order' => $sortOrder,
            'options' => $options,
            'file_rule' => $fileRule,
            ...
            'secondary_tutorial_video_upload' => $secondaryTutorialVideoUpload,
            'secondary_hosted_tutorial_video_filename' => $secondaryHostedTutorialVideoFilename,
            'remove_secondary_hosted_tutorial_video' => $removeSecondaryHostedTutorialVideo,
...
function validate_asset_payload(array $input, ?int $assetId = null, array $fileBag = [], bool $skipFileFields = false): array
{
    ...
    $errors = [];
    $existingAsset = $assetId ? get_asset($assetId, true) : null;
    ...
    if ($assetId === null && $categoryId > 0 && !asset_common_category_allows_manual_rows($segmentId, $categoryId)) {
        $errors['category_id'] = 'This category uses fixed common rows. Add new rows is not allowed here.';
    }
    if ($existingAsset
        && !asset_common_asset_is_generated($existingAsset)
        && $categoryId > 0
        && !asset_common_category_allows_manual_rows($segmentId, $categoryId)
        && (int)($existingAsset['category_id'] ?? 0) !== $categoryId
    ) {
        $errors['category_id'] = 'This category uses fixed common rows. Manual rows cannot be moved into it.';
    }
    ...
    foreach ($fieldMap as $fieldKey => $field) {
        ...
        $resolvedChild = asset_resolve_conditional_child_value($parentField, $parentValue, $childValue);
        if ($resolvedChild === null) {
            $errors[$fieldKey] = ($field['label'] ?? $fieldKey) . ' has an invalid option for the selected ' . ($parentField['label'] ?? $parentField['field_key']) . '.';
            continue;
        }
        $values[$fieldKey]['value_option'] = $resolvedChild;
        $values[$fieldKey]['display'] = $resolvedChild;
    }
    if ($existingAsset) {
        if (asset_common_asset_is_generated($existingAsset)) {
            $categoryId = (int)($existingAsset['category_id'] ?? $categoryId);
            $subcategoryId = (int)($existingAsset['subcategory_id'] ?? $subcategoryId);
        }
        foreach (asset_common_locked_field_keys_for_asset($existingAsset) as $fieldKey) {
            if (!isset($fieldMap[$fieldKey]) || ($fieldMap[$fieldKey]['data_type'] ?? '') === 'file') {
                continue;
            }
...
function normalize_asset_field_value(array $field, mixed $raw, array $fieldMap, array &$errors): array
{
    ...
    if ($type === 'dropdown' || $type === 'conditional') {
        $resolvedOption = asset_resolve_field_option_value($field, $value);
        if ($resolvedOption === null) {
            $errors[$key] = "{$label} has an invalid option.";
            return $normalized;
        }
        $normalized['value_option'] = $resolvedOption;
        $normalized['display'] = $resolvedOption;
        return $normalized;
    }
...
function asset_normalize_unique_compare_value(array $field, mixed $raw): ?string
{
    ...
    return match ($type) {
        ...
        'dropdown', 'conditional' => asset_resolve_field_option_value($field, $value),
...
function create_asset(array $validated, array $user): int
{
    ...
    try {
        ...
        db()->commit();
        $createdAsset = get_asset($assetId, true);
        if ($createdAsset) {
            asset_filter_index_sync_asset_change(null, $createdAsset);
            asset_sync_common_rows_after_asset_change(null, $createdAsset, (int)($user['id'] ?? 0));
        }
...
function persist_asset_record(array $validated, array $user, array &$fileCleanup = []): int
{
    ...
    $profileId = asset_common_profile_id_for_category(
        asset_normalize_segment_id((int)($validated['segment_id'] ?? 0)),
        (int)($validated['category_id'] ?? 0)
    );
    $stmt = db()->prepare('INSERT INTO assets (segment_id, asset_number, category_id, subcategory_id, common_profile_id, is_user_added_row, office_type, office_id, active_status, created_by, created_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?, 1, ?, NOW())');
    $stmt->execute([
        asset_normalize_segment_id((int)($validated['segment_id'] ?? 0)),
        'PENDING',
...
function update_asset(int $assetId, array $validated, array $user): void
{
    ...
    $fileCleanup = ['new_paths' => [], 'delete_paths' => []];
    $beforeAsset = $asset;
    $profileId = asset_common_asset_is_generated($asset)
        ? (int)($asset['common_profile_id'] ?? 0)
        : (asset_common_profile_id_for_category($segmentId, (int)($validated['category_id'] ?? 0)) ?? null);
    db()->beginTransaction();
    try {
        $stmt = db()->prepare('UPDATE assets SET category_id = ?, subcategory_id = ?, common_profile_id = ?, updated_by = ?, updated_at = NOW() WHERE id = ?');
        $stmt->execute([
            $validated['category_id'] > 0 ? $validated['category_id'] : null,
            $validated['subcategory_id'] > 0 ? $validated['subcategory_id'] : null,
            $profileId,
        ...
        $afterAsset = get_asset($assetId, true);
        asset_filter_index_sync_asset_change($beforeAsset, $afterAsset);
        asset_sync_common_rows_after_asset_change($beforeAsset, $afterAsset, (int)($user['id'] ?? 0));
        finalize_asset_file_changes($fileCleanup, true);
...
{
    $assetSegmentStmt = db()->prepare('SELECT segment_id FROM assets WHERE id = ? LIMIT 1');
    $assetSegmentStmt->execute([$assetId]);
    $assetSegmentId = (int)($assetSegmentStmt->fetchColumn() ?: 0);
    $fieldMap = asset_field_map_for_segment(true, $assetSegmentId > 0 ? $assetSegmentId : null);
...
function soft_delete_assets(array $assetIds, array $user): int
{
    ...
    foreach ($assetIds as $assetId) {
        ...
        asset_filter_index_sync_asset_change($asset, null);
        asset_sync_common_rows_after_asset_change($asset, null, (int)($user['id'] ?? 0));
...
function asset_hierarchy_sql_scope(array $filters): ?array
{
    ...
    if ($subdivisionId > 0) {
        return [5 => [$subdivisionId]];
    }

    if ($divisionId > 0) {
        $subdivisionIds = [];
        foreach (asset_office_hierarchy_maps()['subdivisions'] as $row) {
            if ((int)($row['division_id'] ?? 0) === $divisionId) {
                $subdivisionIds[] = (int)$row['id'];
            }
        }
        return [
            4 => [$divisionId],
    ...
    if ($circleId > 0) {
        $maps = asset_office_hierarchy_maps();
        $divisionIds = [];
        foreach ($maps['divisions'] as $row) {
            if ((int)($row['circle_id'] ?? 0) === $circleId) {
                $divisionIds[] = (int)$row['id'];
            }
        }
        $subdivisionIds = [];
        foreach ($maps['subdivisions'] as $row) {
            if ((int)($row['circle_id'] ?? 0) === $circleId) {
                $subdivisionIds[] = (int)$row['id'];
            }
        }
    ...
    if ($zoneId > 0) {
        $maps = asset_office_hierarchy_maps();
        $circleIds = [];
        foreach ($maps['circles'] as $row) {
            if ((int)($row['zone_id'] ?? 0) === $zoneId) {
                $circleIds[] = (int)$row['id'];
            }
        }
        $divisionIds = [];
        foreach ($maps['divisions'] as $row) {
...
function asset_matches_dynamic_filters(array $asset, array $filters, array $fieldMap): bool
{
    foreach ($fieldMap as $fieldKey => $field) {
        ...
        if ($fieldType === 'conditional') {
            ...
            if ($value !== '' && $value !== '__blank__' && !asset_option_values_match($currentValue, $value)) {
                return false;
            }
            continue;
        }
...
function build_asset_filter_catalog(array $assets, array $fields, ?int $segmentId = null, bool $respectBoardFilterVisibility = true, bool $preferIndexedTrackable = false): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    $catalog = [
        'categories' => [],
    ...
    $fieldMetaMap = [];
    $indexedFieldCatalog = ($preferIndexedTrackable && is_superadmin())
        ? asset_filter_index_catalog_for_fields($segmentId, $fields)
        : [];
    foreach ($fields as $field) {
        $fieldKey = (string)($field['field_key'] ?? '');
        if ($fieldKey === '' || empty($visibleFields[$fieldKey]) || asset_is_conditional_secondary($field) || (int)($field['active_status'] ?? 0) !== 1) {
            continue;
        }
        if (isset($indexedFieldCatalog[$fieldKey])) {
            continue;
        }
        $fieldMetaMap[$fieldKey] = [
            'field' => $field,
            'field_type' => (string)($field['data_type'] ?? 'text'),
            'child_field' => (string)($field['data_type'] ?? '') === 'conditional'
                ? get_asset_conditional_child_field((int)$field['id'], true, $segmentId)
                : null,
        ];
    }
    foreach ($indexedFieldCatalog as $fieldKey => $fieldCatalog) {
        $catalog['fields'][$fieldKey] = $fieldCatalog;
    }
    $officeMaps = asset_office_hierarchy_maps();
    foreach ($assets as $asset) {
        $officeType = (int)($asset['office_type'] ?? 0);
        $officeId = (int)($asset['office_id'] ?? 0);
        if ($officeType === 2) {
            ...
            $circle = $officeMaps['circles'][$officeId] ?? null;
            if ($circle) {
            ...
            $division = $officeMaps['divisions'][$officeId] ?? null;
            if ($division) {
            ...
            $subdivision = $officeMaps['subdivisions'][$officeId] ?? null;
            if ($subdivision) {
        ...
        foreach ($fieldMetaMap as $fieldKey => $fieldMeta) {
            $field = $fieldMeta['field'];
            $fieldType = $fieldMeta['field_type'];
            ...
            if ($fieldType === 'file') {
                ...
                $primaryValue = asset_resolve_conditional_primary_value($field, asset_filter_value($asset, $fieldKey)) ?? asset_filter_value($asset, $fieldKey);
                if ($primaryValue === '') {
                    $catalog['fields'][$fieldKey]['has_blank'] = true;
                }
                $childField = $fieldMeta['child_field'];
                if ($childField && (int)($childField['active_status'] ?? 0) === 1) {
                    ...
                    $childValue = asset_resolve_conditional_child_value($field, $primaryValue, asset_filter_value($asset, $childKey)) ?? asset_filter_value($asset, $childKey);
                    if ($childValue === '') {
...
function get_assets(array $filters = [], ?array $user = null, bool $includeDeleted = false, array $options = []): array
{
    $user = $user ?: current_user();
    ...
    $includeFiles = !array_key_exists('include_files', $options) || (bool)$options['include_files'];
    $includeTimestamps = !empty($options['include_timestamps']);
    $includeOfficeLabels = !array_key_exists('include_office_labels', $options) || (bool)$options['include_office_labels'];
    $skipSorting = !empty($options['skip_sort']);
    asset_board_perf_mark('get_assets.start', [
        'segment_id' => $segmentId,
        'include_files' => $includeFiles ? 1 : 0,
        'include_timestamps' => $includeTimestamps ? 1 : 0,
        'skip_sort' => $skipSorting ? 1 : 0,
    ]);
    $sql = 'SELECT a.*, c.name AS category_name, c.sort_order AS category_sort_order, s.name AS subcategory_name, creator.email_id AS created_by_email, editor.email_id AS updated_by_email
            FROM assets a
            LEFT JOIN asset_categories c ON c.id = a.category_id AND c.segment_id = a.segment_id
            LEFT JOIN asset_subcategories s ON s.id = a.subcategory_id AND s.segment_id = a.segment_id
            JOIN users creator ON creator.id = a.created_by
    ...
    $hierarchyScope = asset_hierarchy_sql_scope($filters);
    if ($hierarchyScope !== null) {
        $scopeClauses = [];
        foreach ($hierarchyScope as $scopeOfficeType => $scopeOfficeIds) {
            $scopeOfficeIds = array_values(array_filter(array_map('intval', (array)$scopeOfficeIds), static fn(int $id): bool => $id > 0));
            if ($scopeOfficeIds === []) {
                continue;
            }
            $placeholders = implode(',', array_fill(0, count($scopeOfficeIds), '?'));
            $scopeClauses[] = "(a.office_type = ? AND a.office_id IN ({$placeholders}))";
            $params[] = (int)$scopeOfficeType;
            array_push($params, ...$scopeOfficeIds);
        }
        if ($scopeClauses !== []) {
            $sql .= ' AND (' . implode(' OR ', $scopeClauses) . ')';
        } else {
            return [];
        }
    }
    ...
    $stmt->execute($params);
    $rows = $stmt->fetchAll();
    asset_board_perf_mark('get_assets.query_complete', ['rows' => count($rows)]);

    $assetIds = array_map(fn($row) => (int)$row['id'], $rows);
    $valuesByAsset = get_asset_values_for_assets($assetIds);
    $filesByAsset = $includeFiles ? get_asset_files_for_assets($assetIds) : [];
    $valueTimesByAsset = $includeTimestamps ? get_asset_value_time_map_for_assets($assetIds) : [];
    $fileTimesByAsset = ($includeFiles && $includeTimestamps) ? get_asset_file_time_map_for_assets($assetIds) : [];
    asset_board_perf_mark('get_assets.hydration_complete', [
        'assets' => count($assetIds),
        'files_hydrated' => $includeFiles ? count($filesByAsset) : 0,
        'timestamps_hydrated' => $includeTimestamps ? 1 : 0,
    ]);
    foreach ($rows as &$row) {
        $row['values'] = $valuesByAsset[(int)$row['id']] ?? [];
        ...
        if ($includeOfficeLabels) {
            ...
            $row['office_name'] ??= '';
            $row['office_type_label'] ??= '';
        }
        $row['category_sort_order'] = (int)($row['category_sort_order'] ?? ($row['sort_order'] ?? 0));
        $row['category_name'] = (string)($row['category_name'] ?? ($row['name'] ?? ''));
    }
    unset($row);
    ...
    $rows = array_values(array_filter($rows, static function (array $row) use ($filters, $fieldMap): bool {
        if (!asset_common_generated_superadmin_row_is_currently_applicable($row)) {
            return false;
        }
    ...
    asset_board_perf_mark('get_assets.filters_applied', ['rows' => count($rows)]);
    ...
    if (!$skipSorting) {
        if ($sortColumn !== '') {
            ...
            asset_board_perf_mark('get_assets.sorted', ['column' => $sortColumn, 'direction' => $sortDirection]);
        } else {
            $rows = asset_apply_superadmin_common_default_order($rows);
            asset_board_perf_mark('get_assets.sorted', ['column' => '__common_default__', 'direction' => 'asc']);
        }
    }
    return $rows;
}
...
{
    $isFileColumn = asset_sort_column_is_file($rows, $sortColumn);
    usort($rows, static function (array $left, array $right) use ($sortColumn, $sortDirection, $isFileColumn): int {
        if ($isFileColumn) {
            $comparison = asset_sort_file_count($left, $sortColumn) <=> asset_sort_file_count($right, $sortColumn);
        } else {
...
function asset_sort_column_is_file(array $rows, string $sortColumn): bool
{
    if ($rows === [] || $sortColumn === '') {
        return false;
    }
    if (in_array($sortColumn, ['__sl', 'asset_number', 'office_name', 'subcategory_name', 'data_provider'], true)) {
        return false;
    }
    static $cache = [];
    $segmentId = (int)($rows[0]['segment_id'] ?? 0);
    $cacheKey = $segmentId . ':' . $sortColumn;
    if (array_key_exists($cacheKey, $cache)) {
        return $cache[$cacheKey];
    }
    if ($segmentId > 0) {
        foreach (get_asset_fields(false, $segmentId) as $field) {
            if ((string)($field['field_key'] ?? '') !== $sortColumn) {
                continue;
            }
            $cache[$cacheKey] = (string)($field['data_type'] ?? '') === 'file';
            return $cache[$cacheKey];
        }
    }
    foreach ($rows as $row) {
        if (array_key_exists($sortColumn, (array)($row['files'] ?? []))) {
            $cache[$cacheKey] = true;
            return true;
        }
    }
...
function asset_sort_file_count(array $asset, string $sortColumn): int
{
    return count((array)($asset['files'][$sortColumn] ?? []));
}
...
function asset_group_assets_by_category(array $assets, array $filters = [], ?int $segmentId = null): array
{
    $grouped = [];
    $segmentId = asset_normalize_segment_id($segmentId ?? (isset($filters['segment_id']) ? (int)$filters['segment_id'] : null));
    ...
    $assetsByCategory = [];
    foreach ($assets as $asset) {
        $assetsByCategory[(int)($asset['category_id'] ?? 0)][] = $asset;
    }
    foreach ($categories as $category) {
        $grouped[] = [
            'category' => $category,
            'assets' => array_values($assetsByCategory[(int)$category['id']] ?? []),
        ];
    }
    return $grouped;
}
...
{
    $segmentId = asset_normalize_segment_id(isset($filters['segment_id']) ? (int)$filters['segment_id'] : null);
    $assets = get_assets($filters, $user, false, [
        'include_files' => true,
        'include_timestamps' => false,
        'include_office_labels' => true,
    ]);
    return asset_group_assets_by_category($assets, $filters, $segmentId);
}
...
function asset_download_common_value_for_asset(array $asset, string $identifier, int $segmentId): string
{
    ...
    $value = trim((string)($asset['values'][(string)$field['field_key']] ?? ''));
    if ($value !== '') {
        if ((string)($field['data_type'] ?? '') === 'conditional') {
            $value = asset_resolve_conditional_primary_value($field, $value) ?? $value;
        } elseif (asset_is_conditional_secondary($field)) {
            $parentField = get_asset_field((int)($field['secondary_of_field_id'] ?? 0), $segmentId);
            $parentValue = $parentField ? trim((string)($asset['values'][(string)$parentField['field_key']] ?? '')) : '';
            if ($parentField) {
                $value = asset_resolve_conditional_child_value($parentField, $parentValue, $value) ?? $value;
            }
        } elseif (in_array((string)($field['data_type'] ?? ''), ['dropdown', 'yes_no'], true)) {
            $value = asset_resolve_field_option_value($field, $value) ?? $value;
        }
    }
    return $value === '' ? 'Blank' : $value;
}
...
function asset_audit_asset_label(array $asset): string
{
    ...
    $parts[] = $assetNumber !== '' ? $assetNumber : ('Entry #' . (int)($asset['id'] ?? 0));
    $officeName = trim((string)(asset_download_office_hierarchy($asset)['office_name'] ?? ''));
    if ($officeName !== '') {
        $parts[] = $officeName;
    }
    $categoryName = trim((string)($asset['category_name'] ?? ''));
    if ($categoryName !== '') {
        $parts[] = $categoryName;
    }
...
function asset_audit_sort_labels(array $labels): array
{
...
function asset_audit_office_tree(int $segmentId): array
{
    $overview = get_offices_overview();
    $zoneNodes = [];
    foreach ((array)($overview['zones'] ?? []) as $zone) {
        ...
        $zoneNodes[$zoneId] = [
            'key' => '2:' . $zoneId,
            'label' => trim((string)($zone['office_name'] ?? '')),
            'provided' => false,
            'applicable' => asset_segment_office_type_provides_data(2, $segmentId),
            'has_user' => !empty($zone['linked_user']),
            'office_type' => 2,
            'children' => [],
        ];
    }
    ...
    $circleNodes = [];
    foreach ((array)($overview['circles'] ?? []) as $circle) {
        $circleId = (int)($circle['id'] ?? 0);
        if ($circleId <= 0) {
            continue;
        }
        $circleNodes[$circleId] = [
            'key' => '3:' . $circleId,
            'label' => trim((string)($circle['office_name'] ?? '')),
            'provided' => false,
            'applicable' => asset_segment_office_type_provides_data(3, $segmentId),
            'has_user' => !empty($circle['linked_user']),
            'office_type' => 3,
            'children' => [],
        ];
    }
    ...
    $divisionNodes = [];
    foreach ((array)($overview['divisions'] ?? []) as $division) {
        $divisionId = (int)($division['id'] ?? 0);
        if ($divisionId <= 0) {
            continue;
        }
        $divisionNodes[$divisionId] = [
            'key' => '4:' . $divisionId,
            'label' => trim((string)($division['office_name'] ?? '')),
            'provided' => false,
            'applicable' => asset_segment_office_type_provides_data(4, $segmentId),
            'has_user' => !empty($division['linked_user']),
            'office_type' => 4,
            'children' => [],
        ];
    }
    ...
    $subdivisionBuckets = [];
    foreach ((array)($overview['subdivisions'] ?? []) as $subdivision) {
        $divisionId = (int)($subdivision['division_id'] ?? 0);
        if ($divisionId <= 0) {
            continue;
        }
        $subdivisionBuckets[$divisionId][] = [
            'key' => '5:' . (int)($subdivision['id'] ?? 0),
            'label' => trim((string)($subdivision['office_name'] ?? '')),
            'provided' => false,
            'applicable' => asset_segment_office_type_provides_data(5, $segmentId),
            'has_user' => !empty($subdivision['linked_user']),
            'office_type' => 5,
            'children' => [],
        ];
    }
    ...
    foreach ((array)($overview['divisions'] ?? []) as $division) {
        $divisionId = (int)($division['id'] ?? 0);
        if (!isset($divisionNodes[$divisionId])) {
            continue;
        }
        if (!empty($subdivisionBuckets[$divisionId])) {
            $divisionNodes[$divisionId]['children'] = $subdivisionBuckets[$divisionId];
        }
        $circleId = (int)($division['circle_id'] ?? 0);
        if (isset($circleNodes[$circleId])) {
            $circleNodes[$circleId]['children'][] = $divisionNodes[$divisionId];
        }
    }
    ...
    foreach ((array)($overview['circles'] ?? []) as $circle) {
        ...
        $zoneId = (int)($circle['zone_id'] ?? 0);
        if (isset($zoneNodes[$zoneId])) {
            $zoneNodes[$zoneId]['children'][] = $circleNodes[$circleId];
        }
    }

    return array_values($zoneNodes);
}

function asset_audit_mark_tree(array $nodes, array $providedLookup): array
{
    $marked = [];
    foreach ($nodes as $node) {
        $children = asset_audit_mark_tree((array)($node['children'] ?? []), $providedLookup);
        $node['children'] = $children;
        $node['provided'] = !empty($providedLookup[(string)($node['key'] ?? '')]);
        $marked[] = $node;
    }
    return $marked;
}

function asset_audit_conditional_tree(array $parentField, ?array $childField, array $assets, array $field): array
{
    $map = asset_decode_conditional_map($parentField);
    $parentKey = (string)($parentField['field_key'] ?? '');
    $childKey = (string)($childField['field_key'] ?? '');
    $providedParents = [];
    $providedChildren = [];
    foreach ($assets as $asset) {
        $parentValue = asset_resolve_conditional_primary_value($parentField, trim((string)($asset['values'][$parentKey] ?? ''))) ?? trim((string)($asset['values'][$parentKey] ?? ''));
        $childValue = $childKey !== '' ? (asset_resolve_conditional_child_value($parentField, $parentValue, trim((string)($asset['values'][$childKey] ?? ''))) ?? trim((string)($asset['values'][$childKey] ?? ''))) : '';
        if (!asset_audit_field_has_entry($asset, $field)) {
            continue;
        }
        if ($parentValue !== '') {
            $providedParents[$parentValue] = true;
        }
        if ($parentValue !== '' && $childValue !== '') {
            $providedChildren[$parentValue][$childValue] = true;
        }
    }

    $tree = [];
    foreach ($map as $primaryValue => $secondaryValues) {
        $children = [];
        foreach ($secondaryValues as $secondaryValue) {
            $children[] = [
                'key' => $primaryValue . '||' . $secondaryValue,
                'label' => $secondaryValue,
                'provided' => !empty($providedChildren[$primaryValue][$secondaryValue]),
                'children' => [],
            ];
        }
        $tree[] = [
            'key' => $primaryValue,
            'label' => $primaryValue,
            'provided' => !empty($providedParents[$primaryValue]),
            'children' => $children,
        ];
    }
    return $tree;
...
function asset_audit_dropdown_universe(array $levelField, array $assets): array
{
    $options = [];
    if ((string)($levelField['data_type'] ?? '') === 'yes_no') {
        $options['Yes'] = 'Yes';
        $options['No'] = 'No';
    }
    foreach (get_asset_field_options((int)$levelField['id'], true) as $option) {
        $label = trim((string)($option['option_label'] ?? $option['option_value'] ?? ''));
        if ($label !== '') {
            $options[$label] = $label;
        }
    }
    $fieldKey = (string)($levelField['field_key'] ?? '');
    foreach ($assets as $asset) {
        $value = trim((string)($asset['values'][$fieldKey] ?? ''));
        if ($value !== '') {
            $value = asset_resolve_field_option_value($levelField, $value) ?? $value;
        }
        if ($value !== '') {
            $options[$value] = $value;
        }
    }
    return asset_audit_sort_labels(array_values($options));
}

function asset_audit_generic_universe(array $assets, array $levelField): array
{
    $fieldKey = (string)($levelField['field_key'] ?? '');
    $labels = [];
    foreach ($assets as $asset) {
    ...
    return asset_audit_sort_labels(array_values($labels));
}

function asset_audit_list_items(array $universe, array $providedLookup): array
{
    $items = [];
    foreach ($universe as $value) {
        $items[] = [
            'label' => $value,
            'provided' => !empty($providedLookup[$value]),
        ];
    }
    return $items;
}

function asset_audit_detail_payload(array $user, string $viewScope, int $segmentId, string $fieldKey, string $levelKey): array
{
    $segment = null;
    foreach (get_asset_segments(false) as $segmentRow) {
        if ((int)($segmentRow['id'] ?? 0) === $segmentId) {
            $segment = $segmentRow;
            break;
        }
    }
    ...
    $field = get_asset_field_by_key($fieldKey, $segmentId);
    if (!$field || (int)($field['active_status'] ?? 0) !== 1) {
        throw new RuntimeException('Field not found.');
    }

    $choices = asset_audit_level_choices($user, $viewScope);
    if (!isset($choices[$levelKey])) {
        throw new RuntimeException('Invalid audit level.');
    }
    $levelLabel = (string)$choices[$levelKey];
    $assets = asset_download_accessible_assets_for_segment($segmentId, $user, $viewScope);
    ...
    if ($levelKey === 'count') {
        ...
        foreach ($assets as $asset) {
            $items[] = [
                'label' => asset_audit_asset_label($asset),
                'provided' => asset_audit_field_has_entry($asset, $field),
            ];
        }
        return [
            'ok' => true,
            'kind' => 'list',
            'segment_name' => (string)($segment['segment_name'] ?? ''),
    ...
    if ($levelKey === '__office__') {
        ...
        foreach ($assets as $asset) {
            ...
            $officeId = (int)($asset['office_id'] ?? 0);
            if ($officeType > 0 && $officeId > 0) {
                $providedLookup[$officeType . ':' . $officeId] = true;
            }
        }
        return [
            'ok' => true,
            ...
            'field_label' => (string)($field['label'] ?? ''),
            'level_label' => $levelLabel,
            'tree' => asset_audit_mark_tree(asset_audit_office_tree($segmentId), $providedLookup),
        ];
    }

    $levelField = asset_download_field_for_label($levelLabel, $segmentId);
    if (!$levelField) {
        return [
            'ok' => true,
            'kind' => 'list',
    ...
    $levelType = (string)($levelField['data_type'] ?? 'text');
    if ($levelType === 'conditional' || asset_is_conditional_secondary($levelField)) {
        $parentField = $levelType === 'conditional'
            ? $levelField
            : get_asset_field((int)($levelField['secondary_of_field_id'] ?? 0), $segmentId);
        if (!$parentField) {
            throw new RuntimeException('Conditional field is not configured correctly.');
        }
        $childField = get_asset_conditional_child_field((int)$parentField['id'], true, $segmentId);
        return [
            'ok' => true,
            'kind' => 'hierarchy',
            'segment_name' => (string)($segment['segment_name'] ?? ''),
            'field_label' => (string)($field['label'] ?? ''),
            'level_label' => $levelLabel,
            'primary_label' => (string)($parentField['label'] ?? ''),
            'secondary_label' => (string)($childField['label'] ?? ''),
            'tree' => asset_audit_conditional_tree($parentField, $childField, $assets, $field),
        ];
    }

    $providedLookup = [];
    $levelFieldKey = (string)($levelField['field_key'] ?? '');
    foreach ($assets as $asset) {
        $levelValue = trim((string)($asset['values'][$levelFieldKey] ?? ''));
        if ($levelValue === '') {
            continue;
        }
        if (asset_audit_field_has_entry($asset, $field)) {
            $providedLookup[$levelValue] = true;
        }
    }

    $universe = in_array($levelType, ['dropdown', 'yes_no'], true)
        ? asset_audit_dropdown_universe($levelField, $assets)
        : asset_audit_generic_universe($assets, $levelField);
    ...
    return [
        ...
        'field_label' => (string)($field['label'] ?? ''),
        'level_label' => $levelLabel,
        'items' => asset_audit_list_items($universe, $providedLookup),
    ];
}
function asset_audit_normalize_mode(string $mode): string
{
    $mode = strtolower(trim($mode));
    return in_array($mode, ['all', 'provided', 'not_provided'], true) ? $mode : 'all';
}

function asset_audit_detail_rows_for_export(array $payload, string $mode): array
{
    $mode = asset_audit_normalize_mode($mode);
    if (($payload['kind'] ?? 'list') === 'hierarchy') {
        if (((string)($payload['level_label'] ?? '')) === 'Office') {
            return asset_audit_flatten_office_tree_rows((array)($payload['tree'] ?? []), $mode);
        }
        return asset_audit_flatten_conditional_tree_rows((array)($payload['tree'] ?? []), $mode);
    }
    $rows = [];
    $sl = 1;
    foreach ((array)($payload['items'] ?? []) as $item) {
        $provided = !empty($item['provided']);
        if ($mode === 'provided' && !$provided) {
            continue;
        }
        if ($mode === 'not_provided' && $provided) {
            continue;
        }
        $rows[] = [
            'sl_no' => $sl++,
            'item' => (string)($item['label'] ?? ''),
            'status' => $provided ? 'Provided' : 'Not Provided',
        ];
    }
    return $rows;
}

function asset_audit_node_status(array $node): string
{
    if (array_key_exists('applicable', $node) && empty($node['applicable'])) {
        return 'NA';
    }
    return !empty($node['provided']) ? 'Provided' : 'Not Provided';
}

function asset_audit_flatten_office_tree_rows(array $nodes, string $mode, array $path = []): array
{
    $rows = [];
    $sl = 1;
    $walk = function (array $treeNodes, array $currentPath) use ($mode, &$rows, &$sl, &$walk): void {
        foreach ($treeNodes as $node) {
            if (!is_array($node) || !asset_audit_tree_node_visible($node, $mode)) {
                continue;
            }
            $status = asset_audit_node_status($node);
            $depthPath = $currentPath;
            $depthPath[] = (string)($node['label'] ?? '');
            if ($mode === 'all' || ($mode === 'provided' && $status === 'Provided') || ($mode === 'not_provided' && $status === 'Not Provided')) {
                $zone = $depthPath[0] ?? '';
                $circle = $depthPath[1] ?? '';
                $division = '';
                if (count($depthPath) >= 4) {
                    $division = ($depthPath[2] ?? '') . ' > ' . ($depthPath[3] ?? '');
                } else {
                    $division = $depthPath[2] ?? '';
                }
                $rows[] = [
                    'sl_no' => $sl++,
                    'zone' => $zone,
            ...
            $walk((array)($node['children'] ?? []), $depthPath);
        }
    };
    $walk($nodes, $path);
    return $rows;
...
function asset_audit_flatten_conditional_tree_rows(array $nodes, string $mode): array
{
    $rows = [];
    $sl = 1;
    foreach ($nodes as $primaryNode) {
        if (!is_array($primaryNode) || !asset_audit_tree_node_visible($primaryNode, $mode)) {
            continue;
        }
        $primary = (string)($primaryNode['label'] ?? '');
        $children = (array)($primaryNode['children'] ?? []);
        if (!$children) {
            $provided = !empty($primaryNode['provided']);
            if ($mode === 'all' || ($mode === 'provided' && $provided) || ($mode === 'not_provided' && !$provided)) {
                $rows[] = [
                    'sl_no' => $sl++,
                    'primary' => $primary,
                    'secondary' => '',
        ...
        foreach ($children as $secondaryNode) {
            if (!is_array($secondaryNode) || !asset_audit_tree_node_visible($secondaryNode, $mode)) {
                continue;
            }
            $provided = !empty($secondaryNode['provided']);
            if ($mode === 'all' || ($mode === 'provided' && $provided) || ($mode === 'not_provided' && !$provided)) {
                $rows[] = [
                    'sl_no' => $sl++,
                    'primary' => $primary,
                    'secondary' => (string)($secondaryNode['label'] ?? ''),
                    'status' => $provided ? 'Provided' : 'Not Provided',
                ];
            }
        }
...
function asset_audit_tree_node_visible(array $node, string $mode): bool
{
    ...
    $status = asset_audit_node_status($node);
    if ($mode === 'provided' && $status === 'Provided') {
        return true;
    }
    if ($mode === 'not_provided' && $status === 'Not Provided') {
        return true;
    }
    foreach ($children as $child) {
        if (is_array($child) && asset_audit_tree_node_visible($child, $mode)) {
            return true;
        }
    }
    return false;
...
function asset_audit_detail_visible_count(array $payload, string $mode): int
{
    return count(asset_audit_detail_rows_for_export($payload, $mode));
}

function asset_audit_page_sections(array $user, string $viewScope, string $levelKey): array
{
    $levelValueMap = asset_audit_level_value_map($user, $viewScope);
    $sections = [];
    foreach (asset_audit_segments($user, $viewScope) as $auditSegment) {
        $segment = $auditSegment['segment'];
        $segmentId = (int)($segment['id'] ?? 0);
        $rows = [];
        foreach ((array)($auditSegment['fields'] ?? []) as $index => $field) {
            $rows[] = [
                'sl_no' => $index + 1,
                'field_name' => (string)($field['label'] ?? ''),
                'count' => asset_audit_count_cell((array)($auditSegment['assets'] ?? []), $field, $levelKey, $levelValueMap, $segmentId),
            ];
        }
        $sections[] = [
            'segment_name' => (string)($segment['segment_name'] ?? ''),
...
function asset_audit_export_page_excel(array $user, string $viewScope, string $levelKey): void
{
    ensure_library('PhpOffice\\PhpSpreadsheet\\Spreadsheet', 'PhpSpreadsheet is not installed.');
    $choices = asset_audit_level_choices($user, $viewScope);
    ...
    foreach ($sections as $sheetIndex => $section) {
        ...
        $safeTitle = preg_replace('/[\\\\\\/\\?\\*\\[\\]:]/', '', (string)$section['segment_name']);
        $safeTitle = substr((string)$safeTitle, 0, 31);
        $sheet->setTitle($safeTitle !== '' ? $safeTitle : ('Segment ' . ($sheetIndex + 1)));
        $sheet->setCellValue([1, 1], 'Audit Level');
        $sheet->setCellValue([2, 1], $levelLabel);
        $col = 1;
    ...
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment; filename="audit_' . asset_download_safe_name($levelLabel, 'row_count') . '.xlsx"');
    header('Cache-Control: max-age=0');
...
function asset_audit_export_page_pdf(array $user, string $viewScope, string $levelKey): void
{
    $choices = asset_audit_level_choices($user, $viewScope);
    $levelLabel = (string)($choices[$levelKey] ?? 'Row_count');
    $sections = asset_audit_page_sections($user, $viewScope, $levelKey);
    ...
    foreach ($sections as $section) {
        ...
        foreach ((array)($section['rows'] ?? []) as $row) {
            $html .= '<tr><td>' . e((string)($row['sl_no'] ?? '')) . '</td><td>' . e((string)($row['field_name'] ?? '')) . '</td><td>' . e((string)($row['count'] ?? '')) . '</td></tr>';
        }
        $html .= '</tbody></table>';
    }
    $html .= '</body></html>';
    export_pdf($html, 'audit_' . asset_download_safe_name($levelLabel, 'row_count') . '.pdf', 'portrait');
}

function asset_audit_export_detail_excel(array $payload, string $mode): void
{
    ensure_library('PhpOffice\\PhpSpreadsheet\\Spreadsheet', 'PhpSpreadsheet is not installed.');
    ...
    $secondaryLabel = trim((string)($payload['secondary_label'] ?? '')) ?: 'Secondary Field';
    if ($isOffice) {
        $headers = ['sl_no' => 'SL', 'zone' => 'Zone', 'circle' => 'Circle', 'division' => 'Division', 'status' => 'Provide/Not-provide'];
    } elseif ($isHierarchy) {
        $headers = ['sl_no' => 'SL', 'primary' => $primaryLabel, 'secondary' => $secondaryLabel, 'status' => 'Provide/Not-provide'];
    } else {
    ...
    foreach ($rows as $row) {
        ...
        foreach (array_keys($headers) as $key) {
            ...
            if ($key === 'status' && isset($statusColors[$value])) {
                $sheet->getStyle([$col, $rowNum])->getFont()->getColor()->setARGB($statusColors[$value]);
    ...
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment; filename="audit_detail_' . asset_download_safe_name((string)($payload['field_label'] ?? 'detail')) . '.xlsx"');
    header('Cache-Control: max-age=0');
    $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($book);
    $writer->save('php://output');
    exit;
...
function asset_audit_export_detail_pdf(array $payload, string $mode): void
{
    $rows = asset_audit_detail_rows_for_export($payload, $mode);
    ...
    foreach ($headers as $header) {
        $width = $widths[$index] ?? '';
        $thead .= '<th style="width:' . e($width) . ';">' . e($header) . '</th>';
        $index++;
    }
    $tbody = '';
    foreach ($rows as $row) {
        ...
        $statusColor = '#17324d';
        if ($statusValue === 'Provided') {
            $statusColor = '#11613f';
        } elseif ($statusValue === 'Not Provided') {
            $statusColor = '#a33630';
        } elseif ($statusValue === 'NA') {
            $statusColor = '#1f5fae';
        }
        $tbody .= '<tr>';
        foreach (array_keys($headers) as $key) {
            if ($key === 'status') {
                $tbody .= '<td style="color:' . e($statusColor) . ';font-weight:700;">' . e((string)($row[$key] ?? '')) . '</td>';
            } else {
...
function asset_audit_export(array $user, string $viewScope, array $input): void
{
    $format = strtolower(trim((string)($input['format'] ?? 'excel')));
    if (!in_array($format, ['excel', 'pdf'], true)) {
        $format = 'excel';
    }
    $scope = strtolower(trim((string)($input['scope'] ?? 'page')));
    $levelKey = trim((string)($input['audit_level'] ?? 'count'));
    if ($scope === 'detail') {
        $payload = asset_audit_detail_payload(
            $user,
            $viewScope,
            (int)($input['segment_id'] ?? 0),
            trim((string)($input['field_key'] ?? '')),
            $levelKey
        );
        $mode = asset_audit_normalize_mode((string)($input['detail_mode'] ?? 'all'));
        if ($format === 'pdf') {
            asset_audit_export_detail_pdf($payload, $mode);
        }
        asset_audit_export_detail_excel($payload, $mode);
    }
    if ($format === 'pdf') {
        asset_audit_export_page_pdf($user, $viewScope, $levelKey);
    }
    asset_audit_export_page_excel($user, $viewScope, $levelKey);
}
...
function asset_download_request_from_input(array $input, array $user, string $viewScope = 'my_office'): array
{
    ...
    return [
        ...
        'request' => [
            'output' => $output,
            'download_token' => trim((string)($input['download_token'] ?? '')),
...
function asset_download_completion_cookie_name(): string
{
    return 'asset_download_done';
}

function asset_download_signal_completion(string $token): void
{
    $token = trim($token);
    if ($token === '') {
        return;
    ...
    setcookie(
        asset_download_completion_cookie_name(),
        $token,
        [
            'expires' => time() + 300,
            'path' => '/',
...
function asset_office_hierarchy_maps(): array
{
    static $cache = null;
    if ($cache !== null) {
        return $cache;
    ...
    $zones = [];
    foreach (db()->query('SELECT id, office_name FROM zones')->fetchAll() as $row) {
        $zones[(int)$row['id']] = $row;
    }

    $circles = [];
    foreach (db()->query('SELECT c.*, z.office_name AS zone_name FROM circles c LEFT JOIN zones z ON z.id = c.zone_id')->fetchAll() as $row) {
        $circles[(int)$row['id']] = $row;
    }

    $divisions = [];
    foreach (db()->query('SELECT d.*, c.office_name AS circle_name, z.office_name AS zone_name FROM divisions d LEFT JOIN circles c ON c.id = d.circle_id LEFT JOIN zones z ON z.id = d.zone_id')->fetchAll() as $row) {
        $divisions[(int)$row['id']] = $row;
    }

    $subdivisions = [];
    foreach (db()->query('SELECT s.*, d.office_name AS division_name, c.office_name AS circle_name, z.office_name AS zone_name FROM subdivisions s LEFT JOIN divisions d ON d.id = s.division_id LEFT JOIN circles c ON c.id = s.circle_id LEFT JOIN zones z ON z.id = s.zone_id')->fetchAll() as $row) {
        $subdivisions[(int)$row['id']] = $row;
    }
...
function asset_build_comparison_preview_user(int $officeType, int $officeId): ?array
{
    if (!in_array($officeType, [2, 3, 4, 5], true) || $officeId <= 0) {
        return null;
    }

    $primaryUser = find_primary_office_user($officeType, $officeId);
    if ($primaryUser) {
    ...
    return [
        'id' => 0,
        'email_id' => 'office_preview@local',
        'officer_name' => 'Office Preview',
        'office_role' => 1,
        'office_access_level' => 1,
        'is_primary_office_user' => 1,
        'active_status' => 1,
        'office_type' => $officeType,
        'zone_id' => $scope['zone_id'],
        'circle_id' => $scope['circle_id'],
        'division_id' => $scope['division_id'],
        'subdivision_id' => $scope['subdivision_id'],
    ];
}
...
function asset_template_metadata_sheet_name(array $taken = []): string
{
    return asset_template_safe_sheet_name('row_meta', $taken);
}
...
function asset_template_reorder_input_columns_for_ui(array $columns, ?int $segmentId = null): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    if (!$columns) {
        return [];
    ...
    foreach (['category', 'subcategory'] as $fixedKey) {
        if (!isset($byKey[$fixedKey])) {
            continue;
        }
        $ordered[] = $byKey[$fixedKey];
        $added[$fixedKey] = true;
    }
    ...
    foreach (get_asset_fields(false, $segmentId) as $field) {
        ...
        $fieldKey = (string)($field['field_key'] ?? '');
        if ($fieldKey === '' || !isset($byKey[$fieldKey]) || isset($added[$fieldKey])) {
            continue;
        }
        $ordered[] = $byKey[$fieldKey];
        $added[$fieldKey] = true;
    }
...
function asset_template_input_column_definitions(?int $segmentId = null): array
{
    ...
    return asset_template_reorder_input_columns_for_ui($columns, $segmentId);
}
...
function build_asset_template_autogen_workbook(?int $segmentId = null): \PhpOffice\PhpSpreadsheet\Spreadsheet
{
    ...
    $metaSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet(
        $spreadsheet,
        asset_template_metadata_sheet_name([$dataSheet->getTitle()])
    );
    $dropdownSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet(
        $spreadsheet,
        asset_template_dropdown_sheet_name($segmentId, [$dataSheet->getTitle(), $metaSheet->getTitle()])
    );
    ...
    $templateRows = build_asset_template_rows($segmentId, current_user());
    $commonProfiles = get_asset_common_profiles_for_segment($segmentId, true);
    $commonProfile = count($commonProfiles) === 1 ? $commonProfiles[0] : null;
    $fixedCommonRowsOnly = $commonProfile
        && (string)($commonProfile['row_policy'] ?? '') === asset_common_row_policy_fixed();
    $metaSheet->setCellValue('A1', 'row_number');
    $metaSheet->setCellValue('B1', 'asset_id');
    foreach ($templateRows as $offset => $templateRow) {
        $row = $startRow + $offset;
        if ($row > $endRow) {
            break;
        }
        foreach (($templateRow['values'] ?? []) as $fieldKey => $value) {
            if (!isset($columnLetters[$fieldKey])) {
                continue;
            }
            $dataSheet->setCellValueExplicit(
                $columnLetters[$fieldKey] . $row,
                (string)$value,
                \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING
            );
        }
        if ((int)($templateRow['asset_id'] ?? 0) > 0) {
            $metaSheet->setCellValue('A' . ($offset + 2), $row);
            $metaSheet->setCellValue('B' . ($offset + 2), (int)$templateRow['asset_id']);
        }
    }
    ...
    if ($fixedCommonRowsOnly) {
        $editableRowLimit = min($endRow, $startRow + max(0, count($templateRows)) - 1);
        if ($editableRowLimit >= $startRow) {
            foreach ($inputColumns as $column) {
                $colLetter = $columnLetters[$column['key']];
                $dataSheet->getStyle($colLetter . $startRow . ':' . $colLetter . $editableRowLimit)
                    ->getProtection()
                    ->setLocked(\PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_UNPROTECTED);
            }
        }
        $lockedStartRow = max($startRow, $startRow + count($templateRows));
        if ($lockedStartRow <= $endRow) {
            $dataSheet->getStyle('A' . $lockedStartRow . ':' . $lastColumnLetter . $endRow)->applyFromArray([
                'fill' => [
                    'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
    ...
    foreach ($templateRows as $offset => $templateRow) {
        ...
        foreach ((array)($templateRow['locked_keys'] ?? []) as $fieldKey) {
            if (!isset($columnLetters[$fieldKey])) {
                continue;
            }
            $cellRef = $columnLetters[$fieldKey] . $row;
            $dataSheet->getStyle($cellRef)
                ->getProtection()
                ->setLocked(\PhpOffice\PhpSpreadsheet\Style\Protection::PROTECTION_PROTECTED);
            $dataSheet->getStyle($cellRef)->applyFromArray([
                'fill' => [
                    'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
                    'startColor' => ['rgb' => 'F5F7FA'],
                ],
    ...
    $metaSheet->getProtection()->setPassword('1234');
...
function asset_template_headers(?int $segmentId = null): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    $headers = [
        'category' => 'Category / ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¶ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¿',
...
function asset_template_prefill_rows(?int $segmentId = null, ?array $user = null): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    $ctx = current_office_context($user);
    if ($segmentId <= 0 || !$ctx) {
    ...
    $assets = array_values(array_filter($assets, static function (array $asset) use ($hideUserAddedRows): bool {
        if ((int)($asset['common_profile_id'] ?? 0) <= 0) {
            return false;
        }
        if ($hideUserAddedRows && (int)($asset['is_user_added_row'] ?? 1) === 1) {
            return false;
        }
        return true;
    }));
    ...
    foreach ($assets as $asset) {
        ...
        $values = [];
        if (asset_category_selection_enabled($segmentId)) {
            $values['category'] = (string)($asset['category_name'] ?? '');
        }
        if (asset_subcategory_enabled($segmentId)) {
        ...
        foreach (get_asset_fields(false, $segmentId) as $field) {
            ...
            $fieldKey = (string)$field['field_key'];
            $values[$fieldKey] = (string)($asset['values'][$fieldKey] ?? '');
        }
        $lockedKeys = asset_common_asset_is_generated($asset)
            ? asset_common_locked_field_keys_for_asset($asset)
            : [];
        if (asset_category_selection_enabled($segmentId)) {
            $lockedKeys[] = 'category';
        }
        if (asset_subcategory_enabled($segmentId)) {
            $lockedKeys[] = 'subcategory';
        }
        $rows[] = [
            'asset_id' => $assetId,
            'values' => $values,
            'locked_keys' => array_values(array_unique(array_filter($lockedKeys))),
        ];
    }
...
function build_asset_template_rows(?int $segmentId = null, ?array $user = null): array
{
    return asset_template_prefill_rows($segmentId, $user);
}

function asset_import_metadata_sheet_name(): string
{
    return 'row_meta';
}

function asset_import_extract_metadata_map(string $tmpName): array
{
    ensure_library('PhpOffice\\PhpSpreadsheet\\IOFactory', 'PhpSpreadsheet is not installed.');
    ...
    if (method_exists($reader, 'listWorksheetNames')) {
        try {
            ...
            foreach ($sheetNames as $sheetName) {
                if (strcasecmp(trim((string)$sheetName), asset_import_metadata_sheet_name()) === 0) {
                    $metaSheetName = (string)$sheetName;
                    break;
                }
            }
            if ($metaSheetName !== null && method_exists($reader, 'setLoadSheetsOnly')) {
                $reader->setLoadSheetsOnly([$metaSheetName]);
            } else {
                return [];
            }
        } catch (Throwable $e) {
    ...
    $sheet = $spreadsheet->getSheetByName(asset_import_metadata_sheet_name());
    if (!$sheet) {
        foreach ($spreadsheet->getWorksheetIterator() as $worksheet) {
            if (strcasecmp(trim((string)$worksheet->getTitle()), asset_import_metadata_sheet_name()) === 0) {
                $sheet = $worksheet;
                break;
            }
        }
    ...
    foreach ($rows as $index => $row) {
        ...
        $rowNumber = (int)($row[0] ?? 0);
        $assetId = (int)($row[1] ?? 0);
        if ($rowNumber > 0 && $assetId > 0) {
            $map[$rowNumber] = $assetId;
        }
    }
    return $map;
}

function asset_import_target_asset(int $assetId, ?int $segmentId = null): ?array
{
    if ($assetId <= 0) {
        return null;
    }
    ...
    if ($segmentId !== null && asset_normalize_segment_id((int)($asset['segment_id'] ?? 0)) !== asset_normalize_segment_id($segmentId)) {
        return null;
    }
    return $asset;
}
...
function parse_asset_import_file(string $tmpName, string $originalName, array $user, ?int $segmentId = null): array
{
    ...
    $metadataMap = asset_import_extract_metadata_map($tmpName);
    ...
    foreach ($rows as $rowNumber => $cells) {
        ...
        $importRows[] = stage_asset_import_row($payload, (int)$rowNumber, $segmentId, (int)($metadataMap[(int)$rowNumber] ?? 0));
    }
...
function validate_import_review_row(array $row, ?int $segmentId = null): array
{
    ...
    return stage_asset_import_row($payload, (int)($row['row_number'] ?? 0), $segmentId, (int)($row['target_asset_id'] ?? 0));
}

function stage_asset_import_row(array $input, int $rowNumber, ?int $segmentId = null, int $targetAssetId = 0): array
{
    $segmentId = asset_normalize_segment_id($segmentId);
    ...
    $categoryId = asset_category_selection_enabled($segmentId)
        ? ((int)($targetAsset['category_id'] ?? 0))
        : asset_single_category_id($segmentId);
    if ($categoryName !== '') {
        foreach (get_asset_categories(false, $segmentId) as $category) {
    ...
    $subcategoryId = $subcategoryEnabled ? (int)($targetAsset['subcategory_id'] ?? 0) : 0;
    ...
    return [
        ...
        'target_asset_id' => $targetAsset ? (int)$targetAsset['id'] : 0,
    ...
    if ($validRows) {
        foreach ($validRows as $item) {
            try {
                ...
                if ($targetAssetId > 0) {
                    $targetAsset = asset_import_target_asset($targetAssetId, $segmentId);
                    if (!$targetAsset || !user_can_manage_asset($user, $targetAsset)) {
                        throw new RuntimeException('Import row target is no longer available.');
                    }
                    update_asset($targetAssetId, $item['payload'], $user);
                } else {
