...
if ($page === 'download_asset_common_admin_template') {
...
if ($action === 'save_asset_common_admin_row') {
    if (!can_manage_superadmin_scope()) {
        http_response_code(403);
        exit('Not allowed.');
    }
    try {
        $profileId = input_int('profile_id');
        $rowId = input_int('row_id');
        save_asset_common_admin_row($profileId, [
            'scope_token' => input_str('scope_token', '0:0'),
            'sort_order' => input_int('sort_order'),
            'values' => $_POST['common_row_values'] ?? [],
        ], $rowId > 0 ? $rowId : null);
        flash('success', $rowId > 0 ? 'Common admin row updated.' : 'Common admin row added.');
    } catch (Throwable $e) {
        flash('error', $e->getMessage());
    }
...
if ($action === 'delete_asset_common_admin_row') {
    if (!can_manage_superadmin_scope()) {
        http_response_code(403);
        exit('Not allowed.');
    }
    try {
        delete_asset_common_admin_row(input_int('profile_id'), input_int('row_id'));
...
if ($action === 'upload_asset_common_admin_template') {
    if (!can_manage_superadmin_scope()) {
        http_response_code(403);
        exit('Not allowed.');
    }
    try {
        $summary = import_asset_common_admin_rows_from_template(input_int('profile_id'), $_FILES['template_file'] ?? []);
        flash('success', 'Common rows template imported. Rows processed: ' . (int)($summary['rows_processed'] ?? 0) . '.');
    } catch (Throwable $e) {
        flash('error', $e->getMessage());
    }
    if (input_str('redirect_page') === 'common_fields') {
...
if ($action === 'save_asset_template_source') {
    if (!can_manage_superadmin_scope()) {
        http_response_code(403);
        exit('Not allowed.');
    }
    try {
        set_asset_template_source(input_str('template_source', 'autogenerated'), input_int('segment_id'));
        flash('success', 'Template source updated.');
    } catch (Throwable $e) {
        flash('error', $e->getMessage());
    }
    $adminRedirect();
...
if ($page === 'asset_table_rows') {
    ...
    if (!$scopeAccessAvailable) {
        http_response_code(403);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Not allowed.'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
    ...
    $filters = [
        ...
        'zone_id' => (int)request_str('zone_id', '0'),
        'circle_id' => (int)request_str('circle_id', '0'),
        'division_id' => (int)request_str('division_id', '0'),
        'subdivision_id' => (int)request_str('subdivision_id', '0'),
        'category_id' => (int)request_str('category_id', '0'),
        'office_view_scope' => $currentOfficeViewScope,
        'sort_col' => request_str('sort_col', ''),
        'sort_dir' => strtolower(request_str('sort_dir', 'asc')) === 'desc' ? 'desc' : 'asc',
    ];
    if (asset_subcategory_enabled($activeSegmentId)) {
    ...
    $paginationFilters['category_id'] = 0;
    if (!asset_board_pagination_enabled($paginationFilters, $fields, $activeSegmentId, $timeFilterActive)) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Pagination is available only in the default table view.'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }

    $categoryId = (int)request_str('category_id', '0');
    if ($categoryId <= 0) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Category is required.'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
    ...
    $prefCategoryId = asset_table_preference_category_id($categoryId, $currentOfficeViewScope);
    ...
    $showActionColumn = !is_superadmin() && can_modify_office_assets($user) && !$isUnderMeView;
    ...
    $renderEmptyState = $offset === 0;
    require __DIR__ . '/app/views/board_table_rows_fragment.php';
    $html = ob_get_clean();
    ...
    echo json_encode([
        'ok' => true,
        'html' => $html,
        'loaded_count' => $offset + count($assets),
        'total_count' => count($categoryAssets),
        'has_more' => ($offset + count($assets)) < count($categoryAssets),
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}
