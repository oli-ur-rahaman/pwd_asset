document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-calculation-formula-help]').forEach((button) => {
        button.addEventListener('click', (event) => {
            event.preventDefault();
            event.stopPropagation();
            const modal = document.getElementById('calculation-formula-help-modal');
            if (!modal) {
                return;
            }
            const title = document.getElementById('calculation-formula-help-title');
            const body = document.getElementById('calculation-formula-help-body');
            let lines = [];
            try {
                lines = JSON.parse(button.getAttribute('data-help-lines') || '[]');
            } catch (error) {
            ...
            if (title) {
                title.textContent = button.getAttribute('data-help-title') || 'Calculation Formula Guide';
            }
            if (body) {
                const safeLines = Array.isArray(lines) ? lines : [];
                body.innerHTML = safeLines.length
    ...
    if (importReviewModal && importReviewMeta && importReviewBody) {
        ...
        const createFieldCell = (index, field) => {
            ...
            if (field.data_type === 'calculation') {
                input = document.createElement('input');
                input.type = 'text';
                input.className = 'review-input';
                input.readOnly = true;
                input.placeholder = 'Calculated on save';
            } else if (field.data_type === 'dropdown' || field.data_type === 'yes_no' || field.data_type === 'conditional') {
