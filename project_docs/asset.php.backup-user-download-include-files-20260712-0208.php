    ...
    $stmt = db()->prepare(
        'SELECT r.id, r.profile_id, r.stable_row_key, r.scope_office_type, r.scope_office_id, r.sort_order, r.active_status,
         ...
         ORDER BY r.sort_order ASC,
                  CASE
                      WHEN r.scope_office_type = 0 AND r.scope_office_id = 0 THEN 1
                      WHEN r.scope_office_id = 0 THEN 2
                      ELSE 3
                  END ASC,
                  r.id ASC,
                  v.id ASC'
...
function asset_template_prefill_rows(?int $segmentId = null, ?array $user = null): array
{
    ...
    $stmt = db()->prepare(
        'SELECT a.*, c.name AS category_name, s.name AS subcategory_name
         ...
         WHERE a.segment_id = ?
           ...
           AND a.deleted_at IS NULL
           AND a.active_status = 1
         ORDER BY a.category_id ASC, a.id DESC'
    );
    ...
    $valuesByAsset = get_asset_values_for_assets(array_map(static fn(array $row): int => (int)$row['id'], $assets));
    $rows = [];
    foreach ($assets as $asset) {
        ...
        $asset['values'] = $valuesByAsset[$assetId] ?? [];
        $values = [];
        if (asset_category_selection_enabled($segmentId)) {
